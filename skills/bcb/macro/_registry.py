"""skills/bcb/macro/_registry.py - Mode registry for the macro skill.
...
minimal inline implementation for standalone testing in bcb-sgs-v3/.

This file gives macro its own isolated MODES dict and a stable import
path for mode files:

    from skills.bcb.macro._registry import register_mode

The fallback is ~70 lines and mirrors the real skills._base API surface
(ModeSpec dataclass, make_registry, build_manifest_modes, list_modes,
get_mode, auto_discover_modes, make_route). The sync guard (ensure_fresh)
is intentionally omitted in the fallback - the "sgs" source is not wired
into the real sync_map anyway (documented P2 limitation), so the fallback
behaves identically to the real skills._base when "sgs" is stale.
"""
...
try:
    ...
    # Standalone fallback (no skills._base available - e.g. bcb-sgs-v3/).
    import importlib
    import inspect
...
# Create macro's own isolated MODES dict + register_mode decorator.
