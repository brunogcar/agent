"""skills/ddm/poupanca/helpers.py - Pure-function helpers for the poupanca skill.

Mirrors skills/ddm/juros/helpers.py. _heat_color returns a dict
{text, bg, color} (NOT a string) - this was the bug fix in juros v4 that
the poupanca skill inherits from day one.
"""
...
def _heat_color(value, vmin: float, vmax: float) -> dict:
    """Compute heatmap cell color + text as a dict.

    Returns: {"text": "0,67", "bg": "#eff7ef", "color": "#000"} or
             {"text": "-", "bg": "", "color": ""} for None.

    IMPORTANT: returns a dict (NOT a string). The heatmap_table macro expects
    dicts. This was the bug in juros v1-v3 (string return) - poupanca
    inherits the v4 fix from day one.
    """
    if value is None:
...
def build_observation_rows(observations: list[dict], limit: int = 0) -> list[list]:
    ...
    Poupanca table columns:
      Mes/Ano | Rendimento (%) | Acumulado no ano (%) | Acumulado 12m (%)
    """
    ...
    for o in rows:
        out.append([
            ...
            format_pct(o.get("acumulado_no_ano")),
