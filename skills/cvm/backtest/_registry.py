"""skills/cvm/backtest/_registry.py — Mode registry + auto-discovery.

Central dispatch table for backtest modes. Each mode lives in its own file
...
@dataclass
class ModeSpec:
    """Specification for a backtest mode.

    Attributes:
        name:        Mode name (file name without .py). "run", "strategies", etc.
...
def register_mode(
    ...
    *,
    ...
    """Decorator to register a backtest mode.

    Usage in modes/run.py:
        @register_mode("run", description="...", params={...})
        def run(ticker="", strategy="value_pe") -> dict:
            ...
    """
