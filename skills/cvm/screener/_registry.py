"""skills/cvm/screener/_registry.py — Mode registry + auto-discovery.

Central dispatch table for screener modes. Each mode lives in its own file
...
@dataclass
class ModeSpec:
    """Specification for a screener mode.

    Attributes:
        name:        Mode name (file name without .py). "sector", "compare", etc.
...
def register_mode(
    ...
    *,
    ...
    """Decorator to register a screener mode.

    Usage in modes/sector.py:
        @register_mode("sector", description="...", params={...})
        def sector(setor="", limit=20) -> dict:
            ...
    """
