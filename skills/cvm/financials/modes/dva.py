...
Thin wrapper over `complete(grupo="DVA")` that reshapes the per-period
...
from skills.cvm.financials.modes._statement_sections import (
    dva_section_for, reshape_statement_periods,
)
...
def dva(
    ...
    raw = complete(company=company, period=period, grupo="DVA",
                   consolidado=consolidado, periods=periods)
    if raw.get("status") != "ok":
        return raw

    return reshape_statement_periods(raw, section_fn=dva_section_for,
                                     statement_label="DVA")
