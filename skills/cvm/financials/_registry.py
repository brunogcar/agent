"""skills/cvm/financials/_registry.py — Mode registry + auto-discovery.

Central dispatch table for financials modes. Each mode lives in its own file
...
@dataclass
class ModeSpec:
    """Specification for a financials mode.

    Attributes:
        name:        Mode name (file name without .py). "quarterly", "annual", etc.
...
def register_mode(
    ...
    *,
    ...
    """Decorator to register a financials mode.

    Usage in modes/quarterly.py:
        @register_mode("quarterly", description="...", params={...})
        def quarterly(company="", periods=8, consolidado=1) -> dict:
            ...
    """
