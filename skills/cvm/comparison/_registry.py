"""skills/cvm/comparison/_registry.py — Mode registry + auto-discovery.

Central dispatch table for comparison modes. Each mode lives in its own file
...
Same structure as skills/cvm/financials/_registry.py +
skills/cvm/valuation/_registry.py + skills/cvm/backtest/_registry.py.
...
@dataclass
class ModeSpec:
    """Specification for a comparison mode.

    Attributes:
        name:        Mode name (file name without .py). "side_by_side", "summary", etc.
...
def register_mode(
    ...
    *,
    ...
    """Decorator to register a comparison mode.

    Usage in modes/side_by_side.py:
        @register_mode("side_by_side", description="...", params={...})
        def side_by_side(tickers=None, consolidado=1) -> dict:
            ...
    """
