"""engines/dfc/operating_cf.py -- TTM (trailing twelve months) operating cash flow engine.

Mirrors engines/dre/revenue.py with one change: CVM account code 6.01
(Fluxo de Caixa Operacional / FCO) instead of 3.01 (Receita Liquida).
The TTM derivation algorithm is identical.

FCO lives in the DFC (Demonstração do Fluxo de Caixa) statement group,
...
Example for D = 2024-08-15 (most recent ITR = Q2 2024, data_fim = 2024-06-30):
  ITR 2024 Q2 (meses=6) = cumulative H1 2024 FCO
  ITR 2023 Q2 (meses=6) = cumulative H1 2023 FCO
  DFP 2023 (meses=12)   = full year 2023 FCO
...
TTM FCO computable from: ~2012 onwards (need 2 years of ITR)

FCO values are typically POSITIVE (cash generated from operations).

Standalone module: importable by historical skill + future backtest skill.

Usage:
    from skills.cvm.calculations.engines.dfc.operating_cf import operating_cf_at
    r = operating_cf_at("PETR4", "2024-06-30")  # -> 280000000000.0
"""
...
def operating_cf_at(company: str, date: str) -> float | None:
    ...
    Returns:
        TTM FCO in BRL, or None if data not available.
    """
    dfp = _get_dfp_operating_cf(company)
    itr = _get_itr_operating_cf(company)
...
def operating_cf_periods(company: str) -> list[dict]:
    ...
    Returns a list of {"date": period_end_date, "ttm_fco": value} sorted oldest-first.
    Each entry represents a point where TTM FCO changed (new ITR/DFP filed).

    Useful for building step-function FCO overlays on price charts.
    """
    dfp = _get_dfp_operating_cf(company)
    itr = _get_itr_operating_cf(company)
    ...
    for itr_date in all_itr_dates:
        ttm = operating_cf_at(company, itr_date)
        if ttm is not None:
            periods.append({"date": itr_date, "ttm_fco": ttm})
    ...
    for year, data in sorted(dfp.items()):
        if data["date"] < all_itr_dates[0] if all_itr_dates else True:
            periods.append({"date": data["date"], "ttm_fco": data["value"]})
...
register_engine(EngineSpec(
    name="operating_cf",
    quantity="ttm_fco",
    at_fn=operating_cf_at,
    periods_fn=operating_cf_periods,
    source="DFP (annual) + ITR (quarterly cumulative) DFC 6.01 -- FCO TTM",
    category="dfc",
))
