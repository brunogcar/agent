"""engines/dva_va_received.py -- TTM (trailing twelve months) DVA VA Received engine.
...
CROSS-CHECK
-----------
Total VA to Distribute (7.08, via dva_value_added_at) should approximately
equal dva_net_va_at + dva_va_received_at (7.08 ~= 7.06 + 7.07). Significant
...
Usage:
    from skills.cvm.calculations.engines.dva_va_received import dva_va_received_at
    r = dva_va_received_at("PETR4", "2024-06-30")  # -> 2e9 (positive transfer)
"""
...
DVA_VA_RECEIVED_CODE = "7.07"


def _get_dfp_dva_va_received(company: str) -> dict[str, dict]:
        ...
        rows = conn.execute(
            f"""SELECT c.valor, c.escala, c.data_fim_exerc, e.ano
               ...
               WHERE c.id_empresa IN ({emp_ph})
                 ...
                 AND c.meses = 12
               ORDER BY e.ano DESC""",
            (*empresa_ids, DVA_VA_RECEIVED_CODE),
        ).fetchall()
...
def _get_itr_dva_va_received(company: str) -> dict[str, dict]:
...
@engine_cached
def dva_va_received_at(company: str, date: str) -> float | None:
    ...
    """
    dfp = _get_dfp_dva_va_received(company)
    itr = _get_itr_dva_va_received(company)
...
@engine_cached
def dva_va_received_periods(company: str) -> list[dict]:
    """Get all TTM DVA VA received in transfer (7.07) periods for a company.

    Returns a list of {"date": period_end_date, "ttm_dva_va_received": value}
    sorted oldest-first.
    """
    dfp = _get_dfp_dva_va_received(company)
    itr = _get_itr_dva_va_received(company)
    ...
    for itr_date in all_itr_dates:
        ttm = dva_va_received_at(company, itr_date)
        if ttm is not None:
            periods.append({"date": itr_date, "ttm_dva_va_received": ttm})

    for year, data in sorted(dfp.items()):
        if all_itr_dates and data["date"] < all_itr_dates[0]:
            periods.append({"date": data["date"], "ttm_dva_va_received": data["value"]})
...
register_engine(EngineSpec(
    name="dva_va_received",
    quantity="ttm_dva_va_received",
    at_fn=dva_va_received_at,
    periods_fn=dva_va_received_periods,
