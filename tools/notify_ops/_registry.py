"""Action registry for notify_ops auto-discovery.

Mirrors the pattern in consult_ops/_registry.py and python_ops/_registry.py:
...
- Each action module imports `register_action` and decorates its handler at
  module import time.
- The facade imports `from tools import notify_ops` (which triggers
...
def register_action(
    ...
    Args:
        tool_name: Top-level tool key (e.g. "notify").
        action_name: Action key (e.g. "send", "schedule", "recurring").
