"""Action registry for consult_ops auto-discovery.

Mirrors the pattern in swarm_ops/_registry.py and tavily_ops/_registry.py:
...
- Each action module imports `register_action` and decorates its handler at
  module import time.
- The facade imports `from tools import consult_ops` (which triggers
...
def register_action(
    ...
    Args:
        tool_name: Top-level tool key (e.g. "consult").
        action_name: Action key (e.g. "advise", "review", "explain").
