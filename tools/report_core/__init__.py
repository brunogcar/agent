...
This module ensures that all action handlers in tools/report_core/actions/
...
should be placed elsewhere (e.g., report_core/helpers.py).
"""
import importlib
...
for py_file in Path(__file__).parent.glob("actions/*.py"):
    ...
    if py_file.name != "__init__.py":
        # Construct the full module path: tools.report_core.actions.<stem>
        module_name = f"tools.report_core.actions.{py_file.stem}"
