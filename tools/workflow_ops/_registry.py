"""Action registry for workflow_ops auto-discovery.

Mirrors the pattern in consult_ops/_registry.py, vision_ops/_registry.py,
swarm_ops/_registry.py and tavily_ops/_registry.py:
...
- Each action module imports `register_action` and decorates its handler at
  module import time.
- The facade imports `from tools import workflow_ops` (which triggers
...
[DESIGN] KEY INVARIANTS — read before modifying:
  ...
  4. This registry is the META-LEVEL (what to do: run, list, status, cancel,
     history). A SEPARATE _type_registry.py holds TYPE_DISPATCH (which
     workflow to run: research, data, autocode, ...). The run action
     dispatches into TYPE_DISPATCH.
"""
...
def register_action(
    ...
    Args:
        tool_name: Top-level tool key (e.g. "workflow").
        action_name: Action key (e.g. "run", "list", "status", "cancel", "history").
