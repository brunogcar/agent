"""python_ops — Python execution tool subpackage.

Auto-discovers all action handlers under actions/ at import time so the
DISPATCH dict in _registry.py is populated before the python facade reads
...
Mirrors consult_ops/__init__.py, swarm_ops/__init__.py, and
tavily_ops/__init__.py exactly.
"""
...
for py_file in sorted(_actions_dir.glob("*.py")):
    if py_file.name != "__init__.py":
        module_name = f"tools.python_ops.actions.{py_file.stem}"
