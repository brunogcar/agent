"""Action registry for python_ops auto-discovery.

Mirrors the pattern in consult_ops/_registry.py, swarm_ops/_registry.py,
and tavily_ops/_registry.py:
...
- Each action module imports `register_action` and decorates its handler at
  module import time.
- The facade imports `from tools import python_ops` (which triggers
...
def register_action(
    ...
    Args:
        tool_name: Top-level tool key (e.g. "python").
        action_name: Action key (e.g. "run", "run_data", "eval", "profile", "lint").
