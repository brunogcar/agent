"""vision_ops — Vision meta-tool subpackage.

Auto-discovers all action handlers under actions/ at import time so the
DISPATCH dict in _registry.py is populated before the vision facade reads
...
for py_file in sorted(_actions_dir.glob("*.py")):
    if py_file.name != "__init__.py":
        module_name = f"tools.vision_ops.actions.{py_file.stem}"
