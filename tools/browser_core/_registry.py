"""Auto-registration registry for browser actions.
...
system fully extensible: to add a new browser action, simply:
 1. Create a new file in tools/browser_core/actions/
 2. Define a handler decorated with @register_action("browser", "action_name")
 3. The action is immediately available via the browser() tool
...
# Global dispatch table: {"browser": {"navigate": {"func": ..., "help": ..., "examples": [...]}}}
...
def register_action(
    ...
    """
    Decorator to register a browser action handler function with metadata.

    Args:
        tool_name: Tool namespace. Always "browser" for browser actions.
        action_name: Action identifier exposed to the LLM (e.g., "navigate", "click").
