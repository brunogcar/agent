"""
tools/git.py - Git meta-tool (plugin‑based dispatcher).

All git operations are implemented as separate plugins in the git_ops/ package.
See git_ops/__init__.py for automatic registration.
...
# Importing the package registers all @register_git decorated handlers
from tools import git_ops as _git_plugins
from tools.git_ops._base import REGISTRY
from tools.git_ops._helpers import _resolve_root, _check_repo


# ---------------------------------------------------------------------------
# Dynamic docstring built from registered operations
# ---------------------------------------------------------------------------
...
def _build_doc() -> str:
    lines = [
        ...
        "",
        "operation: " + " | ".join(sorted(REGISTRY.keys())),
        "",
    ...
    # Common usage patterns (kept as a quick reference)
    ...
    if not op_info:
        return {
            ...
            "error": (
                ...
                f"Valid: " + " | ".join(sorted(REGISTRY.keys()))
            ),
        }
...
# Attach the dynamic docstring and mark as MCP tool
