...
NOTE: action names map to python_exec modes:
...
Future AIs: if python_exec gains distinct modes (e.g., "calc" returns
...
def _python(action: str = "", code: str = "", mode: str = "run", **params) -> str:
    ...
    Maps CLI action names to python_exec modes. Currently all actions
    use mode="run" since python_exec does not differentiate execution
    modes. The action parameter is preserved for future mode mapping.
    """
    from tools.python_exec import python

    # Map action names to python_exec modes.
    # Currently all map to "run" — update if python_exec gains modes.
    mode_map = {
        "run": "run",
        "calc": "run",
        "data": "run",
