...
_PATTERNS = [
    ...
    # Cleanup operations
    (r"^cleanup\s+autocode\s+(\d+)\s+days$", "cleanup", "autocode", lambda m: {"days": int(m.group(1))}),
    (r"^cleanup\s+autocode\s+(\d+)$", "cleanup", "autocode", lambda m: {"days": int(m.group(1))}),
    (r"^cleanup\s+autocode$", "cleanup", "autocode", lambda m: {}),
    (r"^clean\s+autocode\s+(\d+)\s+days$", "cleanup", "autocode", lambda m: {"days": int(m.group(1))}),
