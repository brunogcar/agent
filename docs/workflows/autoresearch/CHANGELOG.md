...
| Version | Date | Status |
|---------|------|--------|
| **v1.2.2** | 2026-07-14 | **Phase 4g review — subagent dispatch doc fixes + version sync.** P1-2: Removed incorrect `"falls back to _call() on subagent failure"` claim from v1.1 CHANGELOG entry + AUTORESEARCH.md (no fallback exists — `_call_planner` raises `RuntimeError`, `node_propose` catches it and returns `status="failed"`). P2-2: `WORKFLOW_METADATA["version"]` synced from `"1.2"` to `"1.2.2"` (was stale through v1.2 + v1.2.1). P3-2: `propose.py` top docstring updated — was still describing the v1.0 `_call()` flow, now documents the v1.1+ subagent dispatch + no-fallback policy. Node table row #6 updated (was still saying "via `autocode_impl.helpers._call()`"). Roadmap item #4 clarified (single-subagent dispatch is done; parallel multi-subagent is the remaining future item). |
...
| 6 | `node_propose` | Calls planner LLM via `agent(action="subagent", role="planner")` for isolated curated-context dispatch (v1.1+; was `autocode_impl.helpers._call()` in v1.0). Parses JSON proposal `{description, rationale, new_content}` via `_PROPOSE_JSON_SCHEMA` enforcement (v1.2). |
...
| 4 | Parallel subagent dispatch for proposals | Single-subagent dispatch is done (v1.1). Replace it with PARALLEL subagents (one per hypothesis family) — autocode Future Track F1 pattern. | **P3** |
...
*Last updated: 2026-07-14 (v1.2.2 — Phase 4g review: subagent fallback doc fix + propose docstring + version sync).*
