...
| N5 | ~~**Experiment output logging**~~ | ~~P3~~ | ✅ **Done in v1.8** — `node_run_experiment` writes the FULL stdout+stderr to `{results_path}.d/{iteration}.log` (single mode) or `{iteration}_{i}.log` (parallel mode, one per experiment) BEFORE truncating to 50KB. Operators can inspect the full output for debugging. Non-fatal (disk errors swallowed). |
...
*Last updated: 2026-07-23 (v1.8). See [ARCHITECTURE.md](ARCHITECTURE.md) for file maps, [API.md](API.md) for node details, [INSTRUCTIONS.md](INSTRUCTIONS.md) for AI editing rules.*
