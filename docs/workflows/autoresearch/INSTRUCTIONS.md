...
43. Never write logs to `{results_path}.d/` — use `.autoresearch/logs/` instead (v1.9 D1, user request). The `.d/` suffix was unclear; `.autoresearch/logs/` is more descriptive and matches the project's `logs/` convention.
44. Never record failure memory for parallel losers that improved over the OUTER `current_best` — gate `_record_failure_memory` on `not _is_improvement(metrics[i], current_best, direction)` (v1.9 A1, minimax Bug #1). A loser that DID improve (but lost to the winner) is NOT a failure — recording it would poison procedural memory and mislead future runs.
...
41. Always write the FULL output to `.autoresearch/logs/{iteration}.log` (single) or `{iteration}_{i}.log` (parallel) in `node_run_experiment` BEFORE truncating to 50KB (v1.8 N5). Operators need the full output for debugging when the truncated state copy doesn't have enough context. Non-fatal — wrap in `try/except Exception: pass`.
...
47. Always write 6-col TSV rows with `content_hash` as the 6th column — `f"{iteration}\t{commit}\t{metric}\t{status}\t{safe_desc}\t{safe_hash}\n"` (v1.9 A2, minimax Bug #2). Legacy 5-col ledgers load with `content_hash=""` (backward compatible). Without the 6th column, dedup never fires after resume.
48. Always recompute `consecutive_discards` + `consecutive_no_improvement` on resume — scan the tail of the reloaded history (v1.9 B1, qwen P1-2). Pre-v1.9 these were reset to 0, so the convergence detector wouldn't fire until N MORE discards happened (4+10=14 total instead of 10).
49. Always clean up stale `{project_root}/.autoresearch/parallel/` dirs in `node_setup` BEFORE creating a new branch (v1.9 B2, qwen P1-3). A prior crashed run (SIGKILL/OOM before `node_decide` ran) leaves the dir leaked; accumulating disk usage across many crashes. Best-effort `shutil.rmtree(parallel_dir, ignore_errors=True)` + trace.
50. Always verify `git rev-parse --show-toplevel` matches `Path(project_root).resolve()` before `_git_reset_hard` (v1.9 B3, qwen P1-4). On Windows, `Path(project_root).is_dir()` returns True for junctions pointing to non-git directories; the `.git` check mitigates but if the junction points to a DIFFERENT git repo, the reset would nuke that repo's working tree. Refuse + trace on mismatch.
51. Always stagger parallel `_call_planner` calls by `i*0.5s` (capped at 2.0s) — avoids thundering-herd 429s on rate-limited LLM providers (v1.9 C6, qwen P2-6). The sleep happens INSIDE the submitted function (not the main thread) so all N futures are submitted immediately.
...
> **Don't store `experiment_output` in `experiment_history` entries:** History entries are `{iteration, description, metric, status, commit, content_hash, tokens}` — lean on purpose. Storing full output would bloat the LLM prompt exponentially. **[v1.8 N5]** Operators debugging a specific iteration can now `cat .autoresearch/logs/{iteration}.log` to see the full output (written by `node_run_experiment` BEFORE truncation). The history entry's `tokens` field (v1.8 N6) gives cost context without bloat.
...
> **[v1.9 A1] Don't record failure memory for parallel losers that improved:** First prototype called `_record_failure_memory(...)` on EVERY loser in parallel mode, passing `current_best` = the OUTER (pre-iteration) best. A loser with metric 0.45 (better than outer 0.5, but lost to the winner's 0.40) was stored as "did not improve val_bpb (metric=0.45, best=0.5)" → future runs would recall this and avoid a perfectly valid change. Fix: gate the call on `not _is_improvement(metrics[i], current_best, direction)` — only TRUE losers (metric strictly worse than the outer best) get recorded.

> **[v1.9 A2] Don't lose the dedup hash on resume:** First prototype kept `content_hash` only in the in-memory `experiment_history` (5-col TSV). On resume, `_load_history_from_ledger` parsed 5 cols → every reloaded entry had `content_hash=""` → dedup never fired → the LLM could re-propose the same change endlessly across resumed runs. Fix: add `content_hash` as a 6th TSV column. Legacy 5-col ledgers load with `content_hash=""` (backward compatible — no dedup against old rows, but new rows are protected).

> **[v1.9 A3] Don't use `write_text` for the parallel winner copy:** First prototype used `real_path.write_text(winner_path.read_text(...))` — non-atomic. SIGKILL/OOM between the OS truncating `real_path` and `write_text` finishing left the target_file empty or partial. The next `node_propose` read "" via `_read_target_file` and the LLM proposed a full rewrite of a blank file → chaos. Fix: use `_atomic_write(real_path, winner_path.read_text(...))` — the same helper `node_modify` uses (tempfile + os.fsync + os.replace). Two-line change.

> **[v1.9 D5] Don't use `experiment_count` for `node_reflect`:** First prototype checked `experiment_count % interval == 0`. With `parallel_count=4` and `interval=5`, `experiment_count` jumped 4→8→12→16→20 — 4%5=4, 8%5=3, 12%5=2, 16%5=1, 20%5=0. So reflect only fired every 5 ITERATIONS = every 20 EXPERIMENTS, not every 5 experiments. Worse, with `parallel_count=3` and `interval=5`, `experiment_count` jumps 3→6→9→12→15 — 3%5=3, 6%5=1, 9%5=4, 12%5=2, 15%5=0 → reflect fires every 5 iterations = every 15 experiments. The interval was supposed to be "every N iterations" but was actually "every N×parallel_count experiments". Fix: add `iteration_count` state field (increments by 1 per iteration, NOT by N) and check `iteration_count % interval == 0`.
