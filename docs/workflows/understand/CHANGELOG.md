...
|---------|------|---------|
| v1.9.2 | 2026-07-22 | **General hardening (Claude 2nd review + kgraph fixes).** (1) `ast_parser.py` LRU cache key fixed — `content` param has default `""` so the cache key is `(project_id, content_hash)` only (was: full source string held in LRU = memory bloat). (2) `test_mapper.py` CRITICAL_PATHS updated — `core/memory.py` → `core/memory_engine.py` + `core/memory_backend/store.py` (was: stale path meant memory changes didn't trigger full test suite in autocode). (3) `vectors.py` `_chroma_clients` race fixed — added `threading.Lock` around check-then-create (was: bare check-then-create, concurrent threads could create duplicate PersistentClient). (4) `project.py` `ensure_initialized()` cleanup now runs once per ProjectManager instance via `_cleanup_done` flag (was: ran on every call). (5) JS/TS `../` import resolution depth check — bails with raw `[dep]` if import escapes above project root (was: `Path(".").parent` clamps silently to root = false-positive edges). (6) `report.py` shows `files_skipped` + `stale_pruned` counts. (7) `discover_files` returns `files_skipped` + `stale_pruned` in state. |
...
*Last updated: 2026-07-22 (v1.9.2). See [ARCHITECTURE.md](ARCHITECTURE.md) for file maps, [API.md](API.md) for node details, [INSTRUCTIONS.md](INSTRUCTIONS.md) for AI editing rules.*
