...
- **[v1.4] Understand action routing** — The understand branch in `run_workflow()` now supports `action="index"` (default), `action="query"`, and `action="health"`. Query and health bypass the graph entirely — they call `query_codebase` / `health_check` from `workflows/understand_impl/query.py` directly. This makes the index usable for search without re-running the full 4-node graph. Action is checked BEFORE `build_understand_graph()` is called, so query/health never construct or invoke the LangGraph. `project_root` is validated for ALL actions (query/health still need a real path to construct ProjectManager + look up kg.db). Unknown action returns `status="failed"`.
- **[v1.6] Cancellation across all 3 long-running workflows** — All 3 workflows that benefit from mid-execution cancellation now use it: understand (post-hoc via `is_workflow_cancelled` check after `graph.invoke()` returns), autoresearch (mid-execution via per-node cancellation checks in propose/run_experiment/decide/reflect, added in centralize-workflow-utils Phase B), and autocode (mid-execution via its own module-global `is_cancellation_requested` flag in `workflows/autocode_impl/helpers.py` — documented as legacy; too risky to migrate to `workflows.base.is_workflow_cancelled` in this refactor). The autocode module-global + `invoke_with_timeout()` daemon-thread pattern is preserved; autoresearch's new per-node checks use the base `is_workflow_cancelled` API.
...
*Last updated: 2026-07-25 (v1.6 — autoresearch cancellation integration). See [API.md](API.md) for utility signatures and dispatcher details, [CHANGELOG.md](CHANGELOG.md) for version history, [INSTRUCTIONS.md](INSTRUCTIONS.md) for AI editing rules.*
