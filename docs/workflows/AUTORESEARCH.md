...
**[v1.11] Hardening — Claude review fixes (1 P0 + 3 P1 + 4 P2).** (A3 P0) Parallel mode crashed-subprocess protection — `node_evaluate` marks `proposal["status"]="failed"` when no metric is found, so a crashed subprocess can't win + brick the loop. (A4 P1) `core/backoff_retry.py` gains `non_retryable` param (**standalone v1.6**) — non-transient exceptions propagate immediately (no wasted retry/API hits). (A5 P1) Resume uses `baseline_established: bool` instead of `current_best > 0.0` sentinel — works for negative/zero metrics (log-likelihood, correlation). (A6 P2) Single-path empty-content check moved before dedup hash — correct error reporting. (A7 P2) `run_target_subprocess` uses `Popen` + process-group isolation — timeout kills the whole tree (PyTorch workers, multiprocessing). (A8 P2) `reflect_interval` is now state-overridable (**workflow tool v1.2.2** + **base v1.8**) — also fixes `convergence_window` + `convergence_epsilon` forwarding. (A9 P2) `route_after_log` docstring clarifies experiment-vs-iteration semantics in parallel mode.
...
*Last updated: 2026-07-22 (v1.11). See [CHANGELOG.md](autoresearch/CHANGELOG.md) for version history.*
