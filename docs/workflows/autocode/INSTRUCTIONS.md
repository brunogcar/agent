...
33. **Never bypass the accessor layer when writing NEW code that reads state** — New nodes (and edits to existing nodes) MUST read state via the 8 accessor functions (`_get_plan`, `_get_tdd`, `_get_files`, `_get_impact`, `_get_debug`, `_get_verify`, `_get_vcs`, `_get_memory`) so the eventual legacy-flat-field removal is a mechanical diff.
...
44. **Always use accessor functions (`_get_tdd`, `_get_vcs`, etc.) for state reads in new code** — The 8 accessor functions in `state.py` are the backward-compat layer over the 8 sub-state TypedDicts.
