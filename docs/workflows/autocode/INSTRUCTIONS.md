...
33. **Never bypass the accessor layer when writing NEW code that reads `tdd`, `impact`, or `memory` sub-state** — New nodes (and edits to existing nodes) MUST read these sub-states via their accessors (`_get_tdd`, `_get_impact`, `_get_memory`) so the eventual legacy-flat-field removal is a mechanical diff. **[v2.4 + v2.7] `impact` and `memory` are now SAFE to use** — `node_analyze_impact` writes to the `impact` sub-state via RMW (v2.4), and `node_distill_memory` writes to the `memory` sub-state via RMW (v2.7). **[v2.0.5] WARNING: the other 5 accessors are STILL UNSAFE** — `_get_plan`, `_get_files`, `_get_debug`, `_get_verify`, `_get_vcs` are migration scaffolding that return stale sub-state defaults because nodes write to flat fields, not sub-state dicts. The split-brain trap: `_get_vcs(state, "branch", "main")` returns `""` (the sub-state default), not the actual branch name (which lives in the flat `branch` field). **For all non-migrated reads, use `state.get(key, default)` directly.** Full migration is the v2.x → v3.0 roadmap (see CHANGELOG Future Tracks, Track M1).
...
44. **Always use `_get_tdd(state, key, default)` for TDD sub-state reads in new code** — The `_get_tdd` accessor in `state.py` is safe to use today (nodes actively write to the `tdd` sub-state via read-modify-write). **[v2.4 + v2.7]** `_get_impact` and `_get_memory` are ALSO now safe — `node_analyze_impact` writes to the `impact` sub-state (v2.4), and `node_distill_memory` writes to the `memory` sub-state (v2.7). The other 5 accessors (`_get_plan`, `_get_files`, `_get_debug`, `_get_verify`, `_get_vcs`) are NOT safe — see NEVER DO #33 for the split-brain warning. For all other state reads, use `state.get(key, default)` directly until the v2.x → v3.0 migration is complete.
...
*Last updated: 2026-07-14 (v2.4 + v2.7 — Track M1 Batch 1: impact + memory sub-state migration, accessors now safe; v2.0.5 — Phase 4g review: split-brain accessor warning (#33 + #44); v2.0.4 subagent debug path; v2.0.1 hardening pass; v2.0 GA all 7 phases ✅ COMPLETE). See git history for per-phase details.*
