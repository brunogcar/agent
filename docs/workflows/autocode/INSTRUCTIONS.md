...
33. **Never bypass the accessor layer when writing NEW code that reads `tdd`, `impact`, `memory`, `debug`, `verify`, `vcs`, or `files` sub-state** — New nodes (and edits to existing nodes) MUST read these sub-states via their accessors (`_get_tdd`, `_get_impact`, `_get_memory`, `_get_debug`, `_get_verify`, `_get_vcs`, `_get_files`) so the eventual legacy-flat-field removal is a mechanical diff. **[v2.3] `files` is now SAFE to use** — `apply_patches.py` writes `modified_files`, `write_new_files.py` writes `files_map` + `modified_files`, `execute.py` writes `modified_files`, `brainstorm.py` writes `input_files` (when KG files found). The `_get_files` accessor maps `input_files` → `files` (legacy flat field). Combined with Batches 1+2+3a, **7 of 8 accessors are now safe.** **[v2.0.5] WARNING: the last accessor is STILL UNSAFE** — `_get_plan` is migration scaffolding that returns stale sub-state defaults because nodes write to flat fields, not the `plan_state` sub-state dict. **For all non-migrated reads, use `state.get(key, default)` directly.** Full migration is the v2.x → v3.0 roadmap (see CHANGELOG Future Tracks, Track M1).
...
44. **Always use `_get_tdd(state, key, default)` for TDD sub-state reads in new code** — The `_get_tdd` accessor in `state.py` is safe to use today (nodes actively write to the `tdd` sub-state via read-modify-write). **[v2.1 + v2.3 + v2.4 + v2.5 + v2.6 + v2.7]** `_get_vcs`, `_get_files`, `_get_impact`, `_get_memory`, `_get_debug`, and `_get_verify` are ALSO now safe — `plan.py`/`commit.py`/`push.py`/`create_pr.py` write to the `vcs` sub-state (v2.1), `apply_patches.py`/`write_new_files.py`/`execute.py`/`brainstorm.py` write to the `files_state` sub-state (v2.3), `node_analyze_impact` writes to the `impact` sub-state (v2.4), `node_distill_memory` writes to the `memory` sub-state (v2.7), `node_systematic_debug` writes to the `debug` sub-state (v2.5), and `node_verify_decision` writes to the `verify` sub-state (v2.6). The last accessor (`_get_plan`) is NOT safe — see NEVER DO #33. For all other state reads, use `state.get(key, default)` directly until the v2.x → v3.0 migration is complete.
