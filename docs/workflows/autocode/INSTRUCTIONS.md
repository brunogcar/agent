...
33. **Never bypass the accessor layer when writing NEW code that reads `tdd`, `impact`, `memory`, `debug`, `verify`, or `vcs` sub-state** — New nodes (and edits to existing nodes) MUST read these sub-states via their accessors (`_get_tdd`, `_get_impact`, `_get_memory`, `_get_debug`, `_get_verify`, `_get_vcs`) so the eventual legacy-flat-field removal is a mechanical diff. **[v2.1] `vcs` is now SAFE to use** — `plan.py` writes `branch` to the `vcs` sub-state via RMW, `commit.py` writes `commit_sha`, `push.py` writes `pushed`, `create_pr.py` writes `pr_number` + `pr_url`. The v2.0.5 split-brain band-aid in `commit.py` (direct `state.get("branch")`) has been removed — `_get_vcs` is now the correct reader. Combined with Batches 1+2, **6 of 8 accessors are now safe.** **[v2.0.5] WARNING: the other 2 accessors are STILL UNSAFE** — `_get_plan`, `_get_files` are migration scaffolding that return stale sub-state defaults because nodes write to flat fields, not sub-state dicts. **For all non-migrated reads, use `state.get(key, default)` directly.** Full migration is the v2.x → v3.0 roadmap (see CHANGELOG Future Tracks, Track M1).
...
44. **Always use `_get_tdd(state, key, default)` for TDD sub-state reads in new code** — The `_get_tdd` accessor in `state.py` is safe to use today (nodes actively write to the `tdd` sub-state via read-modify-write). **[v2.1 + v2.4 + v2.5 + v2.6 + v2.7]** `_get_vcs`, `_get_impact`, `_get_memory`, `_get_debug`, and `_get_verify` are ALSO now safe — `plan.py`/`commit.py`/`push.py`/`create_pr.py` write to the `vcs` sub-state (v2.1), `node_analyze_impact` writes to the `impact` sub-state (v2.4), `node_distill_memory` writes to the `memory` sub-state (v2.7), `node_systematic_debug` writes to the `debug` sub-state (v2.5), and `node_verify_decision` writes to the `verify` sub-state (v2.6). The other 2 accessors (`_get_plan`, `_get_files`) are NOT safe — see NEVER DO #33. For all other state reads, use `state.get(key, default)` directly until the v2.x → v3.0 migration is complete.
