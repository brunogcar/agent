...
33. **Never bypass the accessor layer when writing NEW code that reads `tdd` sub-state** — New nodes (and edits to existing nodes) MUST read `tdd` sub-state via `_get_tdd(state, key, default)` so the eventual legacy-flat-field removal is a mechanical diff. **[v2.0.5] WARNING: Only `_get_tdd` is safe to use today** — the other 7 accessors (`_get_plan`, `_get_files`, `_get_impact`, `_get_debug`, `_get_verify`, `_get_vcs`, `_get_memory`) are **migration scaffolding that return stale sub-state defaults** because nodes write to flat fields, not sub-state dicts. Only `tdd` is actively maintained by nodes (via read-modify-write). The split-brain trap: `_get_vcs(state, "branch", "main")` returns `""` (the sub-state default), not the actual branch name (which lives in the flat `branch` field). **For all non-`tdd` reads, use `state.get(key, default)` directly.** Full migration is the v2.x → v3.0 roadmap (see CHANGELOG Future Tracks).
...
44. **Always use `_get_tdd(state, key, default)` for TDD sub-state reads in new code** — The `_get_tdd` accessor in `state.py` is the only accessor safe to use today (nodes actively write to the `tdd` sub-state via read-modify-write). **[v2.0.5]** The other 7 accessors are NOT safe — see NEVER DO #33 for the split-brain warning. For all other state reads, use `state.get(key, default)` directly until the v2.x → v3.0 migration is complete.
...
*Last updated: 2026-07-14 (v2.0.5 — Phase 4g review: split-brain accessor warning (#33 + #44); v2.0.4 subagent debug path; v2.0.1 hardening pass; v2.0 GA all 7 phases ✅ COMPLETE). See git history for per-phase details.*
