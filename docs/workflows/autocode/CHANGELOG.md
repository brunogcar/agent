...
>
> **[v2.0-beta] Update:** `node_write_files`, `node_verify`, AND `node_publish` are now SPLIT (Phase 3 — see ✅ v2.0-beta entries above). The remaining 2 items (debug node statelessness, daemon thread zombie risk) are scheduled for Phase 4 / Phase 7 — see the "2.0 Refactor Progress" section below.
...
**Phase 1 + Phase 2 = v2.0-alpha. Phase 3 = v2.0-beta.** Graph topology changed in
Phase 3 (17 → 27 nodes via 3 splits + 3 backward-compat wrappers). No public API
changes — the wrappers preserve the original node signatures for external callers.
The 3 wrappers (`node_write_files` / `node_verify` / `node_publish`) are registered
in `graph.py` for `import`-compatibility but NOT wired into the graph; they are
excluded from `WORKFLOW_METADATA["nodes"]` so MCP clients render only the 26 active
nodes. `# TODO(2.0):` Phase 6 removes the wrappers once all external callers have
...
*Last updated: 2026-07-11 (v2.0-beta — 2.0 refactor Phase 3 (node decomposition): `node_write_files` split into `apply_patches` + `write_new_files` + `persist_artifacts` (Phase 3.1); `node_verify` split into `run_pytest` + `run_lint` + `llm_review` + `verify_decision` (Phase 3.2); `node_publish` split into `push` + `create_pr` + `merge_pr` (Phase 3.3); original nodes kept as backward-compat wrappers (registered, NOT wired); `_is_path_safe()` moved to `apply_patches.py`; `_build_pr_body` moved to `create_pr.py`; debug loop edges `debug → apply_patches`; `route_after_write_files` / `route_after_run_tests` target `run_pytest`; `route_after_verify` from `verify_decision`; `WORKFLOW_METADATA["version"]` → `"2.0-beta"`; graph registers 27 nodes, metadata lists 26 [wrappers excluded]). See [ARCHITECTURE.md](ARCHITECTURE.md) for file maps, [API.md](API.md) for node details, [INSTRUCTIONS.md](INSTRUCTIONS.md) for AI editing rules.*
