...
| Version | Date | Status |
|---------|------|--------|
| **v2.2** | 2026-07-14 | **Track M1 Batch 3c — plan sub-state migration. 🎉 ALL 8 ACCESSORS NOW SAFE.** Last of the 3 multi-writer sub-states. **State.py fix:** `_default_state()` had `"steps": []` in `plan_state` but `PlanState` TypedDict declares `plan: list[dict]` — pre-existing key mismatch. The accessor worked by accident (always fell through to flat field because `"plan"` key wasn't in sub-state). Fixed: changed `"steps"` to `"plan"` in `_default_state()`. **Writers (4 nodes, 4 RMW blocks):** `brainstorm.py` (1 return — writes `spec`), `plan.py` (1 return — writes `spec` + `plan` + `current_step`), `execute.py` (1 return — writes `current_step`), `tests.py` (1 return — writes `current_step`). All write to the `plan_state` sub-state via RMW alongside legacy flat-field mirrors. **Readers (5 nodes, 14 accessor calls):** `commit.py` (plan — for commit message labels), `execute.py` (plan + current_step), `llm_review.py` (spec), `plan.py` (spec — reader of own field), `tests.py` (plan + current_step + spec). All switched from `state.get("plan", ...)` / `state.get("spec", ...)` / `state.get("current_step", ...)` to `_get_plan(state, key, default)`. **The `plan`/`plan_state` key disambiguation:** `plan` in `AutocodeState` is overloaded — legacy code reads it as `list[dict]` (step list). The sub-state lives under `plan_state` (not `plan`) to avoid breaking list readers. The `_get_plan` accessor reads from `plan_state` first, falls back to flat `plan` field. After this migration, `plan_state["plan"]` holds the step list (same as the flat `plan` field). **Tests:** `conftest.py::base_state` updated to write `plan` + `current_step` to BOTH sub-state + flat field. `test_execute.py` — 3 tests updated (`test_empty_plan_returns_error` adds `plan_state` to hand-built state; `test_increments_current_step` + `test_does_not_increment_beyond_plan` override `plan_state["plan"]` alongside flat `plan`). **🎉 MILESTONE: ALL 8 ACCESSORS NOW SAFE.** `_get_tdd`, `_get_impact`, `_get_memory`, `_get_debug`, `_get_verify`, `_get_vcs`, `_get_files`, `_get_plan`. The v2.x → v3.0 migration track (Track M1) is complete — v3.0 (flat-field removal) is now possible. |
| **v2.3** | 2026-07-14 | **Track M1 Batch 3b — files sub-state migration.** Second of the 3 multi-writer sub-states. **Writers (4 nodes, 5 RMW blocks):** `apply_patches.py` (2 returns: dry_run + success — writes `modified_files`), `execute.py` (1 RMW at return — writes `modified_files` from 3 branches, conditional on `if "modified_files" in updates`), `write_new_files.py` (1 return — writes `files_map` + `modified_files`, merges existing via `_get_files` accessor), `brainstorm.py` (1 return — writes `input_files` when KG files found). All write to the `files_state` sub-state via RMW alongside legacy flat-field mirrors. **Readers (9 nodes, 22 accessor calls):** `analyze_impact.py` (files_map), `brainstorm.py` (input_files — 3 call sites), `debug.py` (modified_files), `memory.py` (modified_files), `plan.py` (input_files), `report.py` (files_map), `run_lint.py` (modified_files), `validate.py` (input_files), `write_new_files.py` (modified_files — reads own field before merging, P1.8 hardening preserved). All switched from `state.get("files", ...)` / `state.get("files_map", ...)` / `state.get("modified_files", ...)` to `_get_files(state, key, default)`. **Accessor already had the legacy_map:** `_get_files` maps `input_files` → `files` (legacy flat field). No state.py change needed. **P1.8 hardening preserved:** `write_new_files.py` still merges `existing_modified + written_files` via set union — now reads existing from `_get_files` accessor instead of `state.get("modified_files", [])`. **Tests:** No test changes needed — `base_state` fixture (from Batch 2) already uses `_default_state()` which populates `files_state` with defaults. Tests that check `"files_map" in result` or `"modified_files" not in result` still pass because flat-field mirrors are kept. **7 of 8 accessors now safe:** `_get_tdd`, `_get_impact`, `_get_memory`, `_get_debug`, `_get_verify`, `_get_vcs`, `_get_files`. Remaining 1 (`_get_plan`) is Batch 3c. |
| **v2.1** | 2026-07-14 | **Track M1 Batch 3a — vcs sub-state migration.** First of the 3 multi-writer sub-states (vcs, files, plan). **Writers (4 nodes, 12 RMW return paths):** `commit.py` (3 returns: skipped, dry_run, done — writes `commit_sha`), `plan.py` (1 return — writes `branch`), `push.py` (3 returns: not-enabled, no-branch, success — writes `pushed`), `create_pr.py` (5 returns: not-enabled, not-pushed, no-branch, success, failure — writes `pr_number` + `pr_url`). All write to the `vcs` sub-state via RMW alongside legacy flat-field mirrors. **Readers (7 nodes, 21 accessor calls):** `commit.py` (branch — removes the v2.0.5 band-aid `state.get("branch")`), `create_pr.py` (commit_sha, pushed, branch), `push.py` (branch), `branch.py` (branch — was `state["branch"]` direct access), `report.py` (commit_sha), `debug.py` (pr_number — 2 call sites), `merge_pr.py` (pr_number). All switched from `state.get(...)` / `state[...]` to `_get_vcs(state, key, default)`. **The v2.0.5 split-brain band-aid is removed:** `commit.py` line 54 was `state.get("branch") or state.get("branch_name") or "main"` (reverted from `_get_vcs` because it was broken). Now that `plan.py` writes to the `vcs` sub-state via RMW, `_get_vcs` is safe and the band-aid is replaced with `_get_vcs(state, "branch", "") or _get_vcs(state, "branch_name", "") or "main"`. **Tests:** `test_branch.py` — 2 tests rewritten to use `_default_state()` + write to both sub-state + flat field (Track M1 learning #2). `test_facade.py` — 3 tests updated to include `verify` + `vcs` sub-states so accessors find the values (sub-state is primary). **6 of 8 accessors now safe:** `_get_tdd`, `_get_impact`, `_get_memory`, `_get_debug`, `_get_verify`, `_get_vcs`. Remaining 2 (`_get_files`, `_get_plan`) are Batch 3b + 3c. |
| **v2.5 + v2.6** | 2026-07-14 | **Track M1 Batch 2 — debug + verify sub-state migration.** Second batch under the v2.x → v3.0 accessor-migration track. **v2.5 (debug):** `node_systematic_debug` (debug.py) — all 5 return paths now write to the `debug` sub-state via RMW alongside legacy flat mirrors (`root_cause`, `defense_notes`, `debug_notes`, `swarm_verdict`, `subagent_verdict`). Added `legacy_map` to `_get_debug` accessor (`notes` → `debug_notes`). 4 reader nodes switched to `_get_debug`: `commit.py` (defense_notes), `create_pr.py` (root_cause, swarm_verdict), `memory.py` (root_cause, defense_notes), `persist_artifacts.py` (debug_notes, root_cause, defense_notes). **v2.6 (verify):** `node_verify_decision` — both 2 return paths now write to the `verify` sub-state via RMW alongside legacy flat mirrors (`verification_passed`, `verification_notes`). 5 reader nodes switched to `_get_verify`: `commit.py`, `create_pr.py`, `merge_pr.py`, `push.py` (all verification_passed), `report.py` (verification_notes). **State.py:** `_get_debug` accessor gained `legacy_map = {"notes": "debug_notes"}` (was identity — `notes` key didn't match the legacy `debug_notes` flat field). **Tests:** `conftest.py::base_state` fixture rewritten to use `_default_state()` instead of a hand-built minimal dict (Track M1 learning #2 — catches split-brain). All verify/debug tests now exercise the real accessor code path with sub-states populated. **Ephemeral fields NOT migrated:** `_pytest_output`, `tests_passed`, `lint_passed`, `llm_review_data` remain flat — they are inter-node ephemeral outputs, not verification results. **5 of 8 accessors now safe:** `_get_tdd`, `_get_impact`, `_get_memory`, `_get_debug`, `_get_verify`. Remaining 3 (`_get_vcs`, `_get_files`, `_get_plan`) are Batch 3. |
