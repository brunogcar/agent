...
|---------|------|---------|
| **v3.6** | 2026-07-19 | **#35 — incremental zombie fix (cancellation-aware subprocess calls).** `run_pytest.py`, `run_lint.py`, and `run_tests.py` now wrap every `subprocess.run(...)` with three cancellation hooks: (1) **pre-check** `is_cancellation_requested()` before invoking the subprocess — bails immediately if the graph already timed out; (2) **deadline-aware timeout** via `_remaining_timeout(default)` — caps the subprocess timeout at `min(default, remaining_graph_budget)` so it can't outlive the graph deadline by more than ~1s; (3) **post-check** `is_cancellation_requested()` after the subprocess returns — discards results so the daemon thread can exit promptly. New helpers in `helpers.py`: `set_graph_start_time()` (called by `invoke_with_timeout()` at the start of each run), `_remaining_timeout(default)` (computes remaining budget), and `_cancelled()` (shortcut for `is_cancellation_requested()`). Full process-level termination (the `multiprocessing.Process` rewrite) is **still deferred** — see roadmap § #35 below. New tests in `tests/workflows/autocode/test_cancellation_aware.py`. |
...
| # | Feature | Shipped |
|---|---------|---------|
| ✅ #35 (incremental) | **Cancellation-aware subprocess calls** | ✅ **Shipped in v3.6** — `run_pytest.py`, `run_lint.py`, `run_tests.py` now wrap every `subprocess.run(...)` with pre-check + deadline-aware timeout + post-check. New helpers in `helpers.py`: `set_graph_start_time()`, `_remaining_timeout(default)`, `_cancelled()`. Bounds daemon-thread zombie linger to ≤1s past the graph deadline. **Full process-level termination still deferred** (see roadmap #35 below). |
...
|---|---------|-------|----------|
| 35 (full) | `invoke_with_timeout` process-level termination | **✅ v3.6 (incremental)** — `run_pytest.py`, `run_lint.py`, `run_tests.py` now wrap `subprocess.run(...)` with pre-check + deadline-aware timeout + post-check (new helpers `set_graph_start_time()` + `_remaining_timeout()` + `_cancelled()` in `helpers.py`). Bounds daemon-thread zombie linger to ≤1s past the graph deadline. **FULL process-level termination still deferred** — Python's `threading.Thread` doesn't support `Thread.kill()`; the daemon thread can still linger for ~1s after the deadline. The full fix requires re-architecting `invoke_with_timeout()` to use `multiprocessing.Process` (terminate-able) — pickling state across the process boundary, IPC for the result dict, SIGTERM handling, and `spawn`-safe imports on Windows. | P2 |
...
| F7 | **Lazy Dev full audit mode** (`task_type="audit"` whole-repo scan) | **DEFERRED — needs detailed design.** The 7-rung Lazy Dev ladder shipped in v2.0 (per-task), but `task_type="audit"` currently routes through the SAME TDD pipeline as `feature` — it doesn't do a whole-repo scan. Full audit mode needs: (1) a new `node_audit_scan` node that walks `project_root` and batches files through `core.kgraph.ast_parser`; (2) a `node_audit_report` node that summarizes findings (dead code, unused imports, missing types, complexity hotspots); (3) routing changes — `route_after_classify` sends `audit` to the audit pipeline instead of `node_brainstorm`; (4) audit-specific prompts; (5) design decision: does audit skip TDD? (current keeps TDD; a full-scan would bypass TDD because there's no single "feature under test"); (6) dependency: kgraph coverage — if the project isn't indexed, `get_callers` returns empty and blast-radius is meaningless. Estimated complexity: **L** (new nodes + routing + prompts + design decision + kgraph coverage check). | P2 |
...
| #57 | **Per-node dedicated test files** | **Added in v3.2 (collective review).** 14 nodes still lack dedicated per-node test files. The pattern in `tests/workflows/autocode/` is one file per concern; new nodes added in v2.0–v3.2 are tested via integration tests rather than per-node unit tests. Goal: each node has a `test_<node_name>.py` with focused unit tests for its skip-conditions, error paths, and state-update shape. | P2 |
...
*Last updated: 2026-07-19 (v3.6). See git history for per-phase details.*
