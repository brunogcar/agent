...
>
> **[v2.0-rc1] Update:** The remaining "debug node statelessness" item is now FIXED (Phase 4 — see ✅ v2.0-rc1 entries above). `node_systematic_debug` accumulates `debug_history` across iterations; new `node_summarize_context` compresses it before re-entering the loop (closes #37). The only remaining deferred item is `invoke_with_timeout` daemon-thread zombie risk (#35) — scheduled for Phase 7 — see the "2.0 Refactor Progress" section below.
...
**Phase 1 + Phase 2 = v2.0-alpha. Phase 3 = v2.0-beta. Phase 4 = v2.0-rc1.** Graph
...
between `node_systematic_debug` and `node_apply_patches`). No public API changes —
the wrappers preserve the original node signatures for external callers, and the
new `node_summarize_context` is purely additive (it reads `debug_history` and
writes `debug_summary`; no other node's contract changed). The 3 wrappers
(`node_write_files` / `node_verify` / `node_publish`) are registered in `graph.py`
for `import`-compatibility but NOT wired into the graph; they are excluded from
`WORKFLOW_METADATA["nodes"]` so MCP clients render only the 27 active nodes.
`# TODO(2.0):` Phase 6 removes the wrappers once all external callers have
migrated to the split nodes.
