...
| v1.2 (legacy) | 2026-07-08 | JSON schema enforcement. `debug.py` passes `json_schema` via `_call()`. (Note: the v1.2 row dated 2026-07-18 above is the doc-drift + roadmap cleanup batch; this row is the original JSON-schema v1.2.) |
...
| 38 | Human-in-the-Loop (HiTL) approval | Pause graph before `commit` or `create_skill`. Needs design decision: (a) **sync-pause** — block on a `threading.Event` until the MCP client responds (simple but holds the worker thread); (b) **async-checkpoint-resume** — checkpoint state + return a `status="awaiting_approval"` response, then resume from checkpoint on the next `run_workflow(resume=True)` call (complex but doesn't hold threads). | P2 |
| F1 | Parallel subagent debug | Single subagent via `AUTOCODE_SUBAGENT_DEBUG=1` (v2.0.2). Parallel subagents (one per hypothesis, thread pool + aggregation) still future. Now unblocked — v2.0.2 action-level allowlist makes this safe. Design: spawn N subagents with curated context per hypothesis; aggregate verdicts with a voting layer (similar to swarm's HIGH/MEDIUM/LOW confidence). | P2 |
