...
*Last updated: 2026-07-19 (v3.2 — 6-LLM collective review hardening: 5 P0 + 6 P1 + 8 P2 fixes; corrected version-numbering regression — the prior `v1.2`/`v1.3` labels in this CHANGELOG were naming mistakes: v1.2 (2026-07-18) → v3.1.2 (doc-drift + roadmap cleanup batch), v1.3 (F8) → v3.1.3 (symbol offloading); the original pre-v2.0 v1.2 is preserved as `v1.2 (legacy)` dated 2026-07-08; version history re-sorted newest-first; completed roadmap items moved to dedicated ✅ Completed Roadmap Items sub-section; new roadmap items #57 per-node test files + #58 standardize status-check pattern added from collective review).*
