...
> **[v3.5 F1]** Parallel subagent debug — NEW 4th debug path in `node_systematic_debug`: `AUTOCODE_PARALLEL_SUBAGENT_DEBUG=1` (default OFF) + `AUTOCODE_PARALLEL_SUBAGENT_COUNT` (default 3). Generates N distinct hypotheses via `PARALLEL_HYPOTHESES_SYSTEM`, dispatches N subagents in parallel via `ThreadPoolExecutor` (one per hypothesis, `SUBAGENT_VALIDATE_SYSTEM`), aggregates by highest `hypothesis_confidence`. All verdicts stored in `debug.parallel_verdicts`. Mutually exclusive with `AUTOCODE_SWARM_DEBUG` and `AUTOCODE_SUBAGENT_DEBUG`. New `_parallel_subagent_debug()` function. See [CHANGELOG.md](CHANGELOG.md) § v3.5 + [API.md](API.md) § "Parallel Subagent Debug".
...
7. If `cfg.autocode_swarm_debug` is ON, call `_swarm_debug_consensus(system, user, tid)`:
   ...
   - If swarm returns `None` (no providers configured, import failure, consensus exception), falls through to the next debug path.
8. **[v3.5 F1]** If `cfg.autocode_parallel_subagent_debug` is ON, call `_parallel_subagent_debug(system, user, tid, retry_temp, debug_history, current_iteration, state)`:
   - **Hypothesis generation:** `_call(role="planner", system=PARALLEL_HYPOTHESES_SYSTEM.format(count=N), user=user)` returns a JSON array of `N = cfg.autocode_parallel_subagent_count` (default 3) distinct hypotheses `{hypothesis_id, root_cause, proposed_fix, confidence}`.
   - **Parallel dispatch:** `concurrent.futures.ThreadPoolExecutor(max_workers=N)` dispatches N `agent(action="subagent", role="executor", task=hypothesis + debug_context, system=SUBAGENT_VALIDATE_SYSTEM, json_schema=_DEBUG_JSON_SCHEMA)` calls — one per hypothesis. Each subagent validates/refines its hypothesis and returns the standard 4-field JSON.
   - **Aggregation:** sort verdicts by `hypothesis_confidence` DESC; winner = verdicts[0]. All N verdicts stored in `debug.parallel_verdicts` for observability; winner mirrored into `debug.subagent_verdict` for downstream readers.
   - Returns the same partial-update shape as the single-subagent path (RMW `tdd` + `debug`).
   - Falls through to the next debug path if hypothesis generation fails, JSON parse fails, `< 2` hypotheses returned, OR all N subagents fail.
   - Mutually exclusive with `AUTOCODE_SWARM_DEBUG` and `AUTOCODE_SUBAGENT_DEBUG` (NEVER DO #40).
9. If `cfg.autocode_subagent_debug` is ON, call `agent(action="subagent", role="executor", task=user, system=system, json_schema=_DEBUG_JSON_SCHEMA)` (single isolated dispatch). Falls through to single-LLM on subagent failure or unparseable JSON.
10. Otherwise (all opt-in flags OFF, or swarm + parallel + subagent all unavailable), call `llm.complete(role="executor", ..., json_schema=_DEBUG_JSON_SCHEMA)` for single-LLM debug analysis.
11. Parse JSON response for `phase`, `root_cause`, `defense_notes`, and `fix`. Validate `phase` against the allowed enum; default to `"investigation"` on unknown value.
12. If swarm returned LOW confidence AND `cfg.autocode_debug_comment_pr` is ON AND a PR exists (`_get_vcs(state, "pr_number", 0)` is set), post a warning comment on the PR via `_github_pr_comment()`.
13. Append a new entry to `debug_history`: `{iteration: current_iteration, phase: phase, root_cause: root_cause, fix: (suggested_fix or "")[:200], tests_passed: False}` (tests_passed is updated to True by `node_run_tests` on the next loop iteration if the fix worked). Swarm-path entries include extra `confidence` field. **[Hardening P0.1]** Read-modify-write preserves sibling TDD fields.

**Output:** Partial dict with `tdd` (sub-state RMW: writes `source_code` = suggested fix, `debug_history` updated, `status`, `error` on early exits) + `debug` (sub-state RMW: writes `root_cause`, `defense_notes`, `notes`, `swarm_verdict`, `subagent_verdict`, `parallel_verdicts`). When swarm was used, the swarm verdict is in `debug.swarm_verdict`. **[v3.5 F1]** When the parallel path fired, `debug.parallel_verdicts` holds ALL N subagent verdicts (sorted by descending `hypothesis_confidence`) and `debug.subagent_verdict` mirrors the winner. `error` (flat status) on early-exit paths. Both early-exit paths preserve `debug_history` (in `tdd` sub-state).
...
**Fallback chain (v3.5):** `AUTOCODE_SWARM_DEBUG=1` + swarm available → use swarm. `AUTOCODE_PARALLEL_SUBAGENT_DEBUG=1` + hypotheses generated → use parallel subagent. `AUTOCODE_SUBAGENT_DEBUG=1` + subagent returned parseable JSON → use single subagent. Otherwise → single-LLM debug (default). All three opt-in flags are mutually exclusive (NEVER DO #40) — pick ONE per workflow run. Each path falls through to the next on failure (e.g., swarm returns `None` → parallel subagent path; parallel subagent yields no verdicts → single subagent path; single subagent returns unparseable JSON → single-LLM).

**Parallel subagent is non-blocking (v3.5 F1):** `_parallel_subagent_debug()` returns `None` to signal "fall through" if (a) `cfg.autocode_parallel_subagent_count < 2`, (b) the hypothesis-generation `_call()` raises, (c) `extract_json_array()` returns `< 2` hypotheses, or (d) all N dispatched subagents fail (status != "success" or unparseable JSON). All four fall-through paths are traced via `tracer.step(...)`. If at least one subagent succeeded, the highest-`hypothesis_confidence` verdict wins.
...
**Smoke test (v3.5 F1):** The `AUTOCODE_PARALLEL_SUBAGENT_DEBUG` enable/disable contract + aggregation-callable check is covered by `tests/workflows/autocode/test_parallel_subagent.py` (3 classes, 4 tests): `TestParallelSubagentDisabled` (parallel path NOT entered when flag OFF — `_call` not invoked with `PARALLEL_HYPOTHESES_SYSTEM`), `TestParallelSubagentEnabled` (config-flag reading + count env-var override), `TestParallelSubagentAggregation` (`_parallel_subagent_debug` is callable + importable).

**`# TODO(2.0-post):`** items: cross-run procedural memory recall before debug (F5); ~~subagent dispatch for parallel debug (F1)~~ ✅ v3.5; adaptive `_ARCHITECTURE_QUESTION_THRESHOLD` per task type (F4).
