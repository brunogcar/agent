...
*Last updated: 2026-07-18 (v1.2 — doc drift fix: `plan` field description corrected — `plan` is NOT a flat field in v3.0+, it lives in `plan_state["plan"]`; `files` field description updated to remove stale `input_files` reference; `mode` valid-values list corrected to `feature`/`fix`/`fix_error`/`refactor`/`improve`/`edit`/`create_skill`/`audit`; v3.0 — flat-field removal, Track M1 ✅ COMPLETE).*
