...
*Last updated: 2026-07-19 (v3.2 — 6-LLM collective review hardening: `MemoryState.context` field removed (P1-2 — was declared in the TypedDict but never populated by any node); `test_code` type annotation corrected `str` → `list[str]` in `state.py` (P1-1) and consumer handling documented for `node_persist_artifacts` + `node_llm_review`; dead state fields `target_file` (core flat), `error_log` (ephemeral flat), `branch_name` (`VCSState`) removed from documentation (P2-6 — verified no node writes them); `swarm_fallback.py` added to `debug_history` writers list (P0-2 — HIGH-confidence path now appends a `phase="swarm_fallback"` entry to `debug_history` so the next `node_systematic_debug` iteration sees the swarm verdict); v3.1.2 — doc drift fix: `plan` field description corrected — `plan` is NOT a flat field in v3.0+, it lives in `plan_state["plan"]`; `files` field description updated to remove stale `input_files` reference; `mode` valid-values list corrected to `feature`/`fix`/`fix_error`/`refactor`/`improve`/`edit`/`create_skill`/`audit`; v3.0 — flat-field removal, Track M1 ✅ COMPLETE).*
