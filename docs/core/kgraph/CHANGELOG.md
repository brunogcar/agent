...
| Version | Date | Notes |
|---------|------|-------|
| **v1.9** | 2026-07-22 | **General hardening.** (1) `ast_parser.py`: LRU cache key fixed — `content` param has default so cache key is `(project_id, content_hash)` only (memory bloat fix). (2) `test_mapper.py`: CRITICAL_PATHS updated — `core/memory.py` → `core/memory_engine.py` + `core/memory_backend/store.py` (stale path fix). (3) `vectors.py`: `_chroma_clients` check-then-create race fixed with `threading.Lock`. (4) `project.py`: `ensure_initialized()` cleanup runs once per instance via `_cleanup_done` flag. |
| **v1.8** | 2026-07-22 | **Claude review fixes.** `embeddings.py`: cache key includes model name (P1-2); `reset_embedding_check()` called each run by understand (P1-3); HTTP calls wrapped in `backoff_retry` (P2-7). `project.py`: `get_indexing_mode()` collapsed from 3-state to 2-state — `background` removed (was aspirational, never implemented) (P2-5). `vectors.py`: dead `upsert_file_vectors` function deleted (P2-8). |
| **v1.7** | 2026-07-22 | **Cross-language import resolution in `parse_and_store`.** The new `_resolve_import_to_file_paths(dep, rel_path, language)` helper in `workflows/understand_impl/nodes/parse_and_store.py` resolves JS/TS relative imports (`./foo`, `../utils`) to candidate file paths (up to 13 candidates per import: 1 raw + 6 extension variants + 6 index-file variants) before storing them as edge targets in `GraphStore.edges.target_id`. Go package imports (`github.com/foo/bar`) store the package short-name (`bar`) as a derivative. Rust `use` declarations store each `::`-separated path segment except the last item name. Python unchanged (already resolved in v1.4). Non-relative JS/TS imports (`react`, `@scope/pkg`) stored raw only. **Effect on kgraph queries:** `get_dependencies` now returns resolved file paths for JS/TS (was: raw import strings only) — the existing v1.4.1 multi-language filter already accepted any path-like target, so the query layer needed NO changes; it just returns richer results. `get_callers` benefits transitively (more edges to match). Candidates that don't exist on disk never match a stored node, so they're harmless. |
| **v1.6** | 2026-07-22 | **Configurability bundle touches 3 kgraph files.** (1) `embeddings.py` — embedding cache (`_embedding_cache` dict keyed by md5(text), cap 10000 entries, `clear_embedding_cache()` for testing, `_maybe_evict_cache()` internal). `embed_texts()` gained an optional `model: str = ""` parameter for per-project overrides (backward compat: empty string uses `cfg.embedding_model`). The HTTP call extracted to `_embed_via_http()` so the cache wraps it cleanly. (2) `project.py` — `_DEFAULT_SKIP_DIRS` (renamed from `SKIP_DIRS`); `get_skip_dirs()` classmethod merges with `UNDERSTAND_SKIP_DIRS` env var; `get_embedding_model()` instance method reads `.understand/config.json` override. The `SKIP_DIRS` class attribute is kept as a backward-compat alias to `_DEFAULT_SKIP_DIRS`. (3) `vectors.py` — `upsert_file_vectors()` + `query_similar_code()` pass `pm.get_embedding_model()` to `embed_texts()`. |
| **v1.5** | 2026-07-22 | **Stale-index cleanup support in `GraphStore`.** Two new methods added for understand v1.6's `node_discover_files` Phase 2: (1) `get_all_file_paths(project_id)` — returns all stored file paths for a project (used to compute `orphans = stored_paths - disk_paths`). (2) `delete_file_entry(project_id, path)` — deletes a file's node + all its edges (outgoing via `source_id = file:{path}` + incoming via 3 target_id forms: path verbatim, `file:{path}` node-id form, and Python module-name form like `core.config` for `core/config.py`). The 3-form deletion is needed because `parse_and_store` stores edges with target_ids that vary by importer language (Python module-name, JS relative path, Go package path, etc.). DELETEs that match nothing are no-ops, so over-deletion is harmless; under-deletion would leave orphaned edges. |
...
*Last updated: 2026-07-22 (v1.9). See [ARCHITECTURE.md](ARCHITECTURE.md) for file maps, [API.md](API.md) for component details, [INSTRUCTIONS.md](INSTRUCTIONS.md) for AI editing rules.*
