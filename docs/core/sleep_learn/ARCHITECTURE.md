...
- **v1.5 (Bug 1): Injector split-brain ACTUALLY fixed** — `get_relevant_rules()` was querying the legacy `procedural_meta` collection unconditionally AND the unified `procedural` collection. Now the legacy query is gated behind `if not SLEEP_LEARN_UNIFIED:` — when `SLEEP_LEARN_UNIFIED=true` (default), ONLY `memory.recall(collections=["procedural"])` is queried. The v1.0 (Commit 4) note claimed this was resolved, but the code didn't match until v1.5. Also fixed: literal `"daemon"` trace_id → `generate_trace_id()`; confidence read order canonical-first (`meta.get("confidence", meta.get("confidence_score", 0.0))`).
- **v1.5 (Bug 2): Feedback confidence field fix** — `update_rule_confidence()` now writes BOTH `meta["confidence"]` (canonical) AND `meta["confidence_score"]` (legacy mirror). Was only writing `confidence_score`, so the injector's canonical `meta.get("confidence", ...)` read saw stale data (boosts/penalties never reached the Planner). Reads canonical first: `meta.get("confidence", meta.get("confidence_score", 0.8))`.
- **v1.5 (Bug 5): `reasoning` field populated end-to-end** — `procedural/distill.py` now requires `reasoning` in `_DISTILL_JSON_SCHEMA` (properties + required) and threads it through `store_procedural(reasoning=...)` → `build_unified_metadata()`. The injector surfaces it in the Planner prompt ("Reason: ...").
...
- **v1.5 (Bug 4): Idle detection extended to `meta_learning.py`** — `MetaLearner.run_forever()` was using `_time.sleep(1800)` unconditionally (no idle check, and `time.sleep` is globally mockable — breaks daemon timing in test environments). Now mirrors `daemon.py`: `tracker.try_acquire_background_slot(min_idle_seconds=300)` + `threading.Event().wait(timeout=1800)` (immune to `time.sleep` mocks) + `tracer.new_trace()` per cycle. Any new background daemon MUST follow this pattern.
...
*Last updated: 2026-07-18 (v1.5: injector unified read path, meta-learner idle detection, feedback confidence fix). See [API.md](API.md) for component details, [CHANGELOG.md](CHANGELOG.md) for version history, [INSTRUCTIONS.md](INSTRUCTIONS.md) for AI editing rules.*
