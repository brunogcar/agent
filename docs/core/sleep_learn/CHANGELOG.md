...
| Version | Date | Notes |
|---------|------|-------|
| **v1.2** | 2026-07-18 | **Post-merge cleanup (minimax review): 12 bugs fixed — 5 P0 + 4 P1 + 3 P2.** Closes the gap between the v1.0 (Commit 4) unified-collection contract and what the writers/injector actually wrote at runtime. **P0:** (1) `injector.py` split-brain ACTUALLY fixed — `SLEEP_LEARN_UNIFIED` flag now gates the legacy `procedural_meta` query; when `true` (default) ONLY the unified `procedural` collection is queried. Also: literal `"daemon"` trace_id → `generate_trace_id()`; confidence read order canonical-first (`meta.get("confidence", meta.get("confidence_score", 0.0))`). (2) `feedback.py update_rule_confidence()` now writes BOTH `meta["confidence"]` (canonical) AND `meta["confidence_score"]` (legacy mirror) — was only writing `confidence_score`, so the injector's canonical read saw stale data (boosts/penalties never reached the Planner). Also reads canonical first; literal `"daemon"` trace_id → `tracer.new_trace("sleep_learn", goal="feedback cycle")`. (3) `meta_learning.py` canonical tags: `"meta-learned,auto-distilled"` → `"source:meta_learner,category:auto_distilled"`; now calls `normalize_tags()` + `validate_tags()` before `memory.store()`. (4) `meta_learning.py` idle detection: `run_forever()` was `_time.sleep(1800)` unconditionally; now mirrors `daemon.py` — `tracker.try_acquire_background_slot(min_idle_seconds=300)` + `Event.wait(timeout=1800)` (immune to `time.sleep` mocks); uses `tracer.new_trace()` per cycle. (5) `distill.py` `reasoning` field added end-to-end: `_DISTILL_JSON_SCHEMA` now requires `reasoning: {type: string, maxLength: 500}`; system prompt asks for it; `store_procedural(reasoning=...)` wired through `_store()` → `execute_store()` → `build_unified_metadata()`. Injector's `meta.get("reasoning", "")` now finds populated data. **P1:** (6) `atomic_extract.py` dedup `min_score=0.0` (fetch all, post-filter) → `0.92` (let ChromaDB filter at the source). (7) `atomic_extract.py` tag prefix `type:{fact['type']}` → `category:{fact['type']}` (`type:` is not in `VALID_TAG_PREFIXES`). (8) `meta_learning.py` docstring "UNIFICATION STATUS: Partial" → "Complete". (9) `update.py` `_get_audit_db()` true singleton (was new SQLite connection per call; double-checked locking via `_audit_conn` + `_audit_lock`; removed 3 `audit_conn.close()` calls). **P2:** (10) `janitor.py` comment: Janitor Bypass is at the ACTION layer (`tools/memory_ops/actions/janitor.py` lazy-imports). (11) `migrate.py` merge semantics documented (confidence=MAX not average, recall_count/last_accessed_at NOT updated, provenance_count recomputed from `source_trace_ids` union — deliberate choices). (12) `read_ops.py` field rename `trace_id` → `source_trace_id` in recall results (memory's ORIGIN trace, not the current query's trace). See [SLEEP_LEARN.md](../SLEEP_LEARN.md) § v1.2 Post-Merge Cleanup for the table view. |
