...
| `heuristics.py` | ~385 | 17 pre-compiled module-level regex patterns (`_RE_*`, Pre-v1.1 added `_RE_DIRECT_SCHEDULE`) + `heuristic_route(goal) -> RoutingDecision` standalone function (19-step priority chain, Pre-v1.1 added step 12b for schedule tasks). Holds the `[DESIGN]` block (priority order, false-positive history, `_RE_RESEARCH` dual-check, hardcoded tool lists). |
