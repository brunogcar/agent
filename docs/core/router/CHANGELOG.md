...
| Version | Date | Notes |
|---------|------|-------|
| **v1.1** | 2026-07-26 | **Cognitive framing + `autoresearch` added to `ROUTER_WORKFLOWS`.** (1) Bug fix: `autoresearch` was missing from `ROUTER_WORKFLOWS` — the router prompt listed only 5 workflows (research, data, autocode, deep_research, understand), so `workflow(type="auto")` could never route to autoresearch (the type handler existed but was unreachable via the auto path). Now `ROUTER_WORKFLOWS` has 6 entries. (2) `ROUTER_SYSTEM_PROMPT` workflow routing rules rewritten with **cognitive-question framing** — each workflow answers a distinct cognitive question ("What is this codebase?" / "What's known externally?" / "What's known (complex)?" / "What approach works best?" / "Execute the change" / "What does the data show?"). (3) New `_RE_AUTORESEARCH` heuristic pattern (`\b(optimize|hyperparameter|experiment loop|autoresearch|evolutionary search)\b`) added as step #14b in `heuristic_route()` — placed BEFORE the generic `_RE_RESEARCH` check at step #17 so autoresearch-specific goals don't fall through to research. (4) New `autoresearch` few-shot example in `ROUTER_FEW_SHOT_EXAMPLES` ("optimize the learning rate of my training script" → workflow=autoresearch). (5) `tests/core/router/conftest.py` `ROUTER_EXPECTED_WORKFLOWS` updated to include `autoresearch` (6 entries); `test_router_drift.py::test_router_workflow_count_is_5` renamed to `_is_6`; `test_router_routing_rules.py::test_workflow_routing_rules_section` relaxed to match the new cognitive-framing section header. |
| **Pre-v1.1** | 2026-07-16 | Added `schedule` to `ROUTER_TOOLS` (17 → 18 tools) + new `_RE_DIRECT_SCHEDULE` heuristic regex pattern (cron job / recurring task / calendar sync / iCal / CalDAV intent) + routing step 12b in `heuristic_route()`. Routes to `schedule` tool (not `notify`) for explicit scheduling intent; simple "remind me" still routes to `notify`. `heuristics.py` grew from 367 → ~385 lines, 16 → 17 regex patterns. |
...
*Last updated: 2026-07-26 (v1.1 — `autoresearch` added to `ROUTER_WORKFLOWS` (was missing — bug fix); router prompt workflow routing rules rewritten with cognitive-question framing; new `_RE_AUTORESEARCH` heuristic pattern at step #14b; new autoresearch few-shot example; tests updated for 6-workflow set). See [ARCHITECTURE.md](ARCHITECTURE.md) for file maps, [API.md](API.md) for method details, [INSTRUCTIONS.md](INSTRUCTIONS.md) for AI editing rules.*
