...
| `core/llm_backend/response.py` | `LLMResponse` dataclass + `ToolCall` dataclass (v1.4: `id`, `name`, `arguments`). `tool_calls` field on `LLMResponse` (default `[]`). v1.4.1: `iterations` (int, loop call count) + `reason` (str, structured bail reason) fields. |
