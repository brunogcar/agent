...
**Returns:** `LLMResponse` with the final text. `tool_calls` is empty (loop consumed them). `usage` is aggregated across all iterations.

**v1.4.1 fields on the returned `LLMResponse`:**
