...
| ------------- | ----------- | ------------------------------------------------------------ | ------ |
| `selic`       | Selic       | Taxa Selic diária (BCB). Taxa média ajustada dos bancos no SELIC. | % a.a. |
| `meta-selic`  | Meta Selic  | Meta para a taxa Selic definida pelo Copom.                  | % a.a. |
| `cdi`         | CDI         | Certificado de Depósito Interbancário. Taxa média dos financiamentos diários no mercado interbank. | % a.a. |

## Why derived (not raw)?

Unlike inflation pages, juros pages ship ONLY the monthly matrix
(`id="index-values"`). There is **no historical table** on the page and
**no "Ano" acumulado column** (these are daily rates, not cumulative
variations). The historical series is therefore **derived** at parse
time from the matrix (see `fetcher.flatten_matrix_to_observations`):

| Field          | Derivation                                                            | Matches Google Sheet formula                                          |
| -------------- | --------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `month_value`  | cell value (daily rate % for that month)                              | `B:B` (raw)                                                           |
| `media_no_ano` | average of all months in same year UP TO that month (year-to-date)    | `AVERAGE(FILTER(B:B, YEAR(A:A)=YEAR(d), A:A<=d))`                     |
| `media_12m`    | average of the last 12 months INCLUDING the current (rolling window)  | `AVERAGE(FILTER(B:B, A:A<=d, A:A>=d-365))`                            |
...
- **Example**: `data_source(domain="ddm", sub_domain="juros", mode="sync_all")`.
...
- **Returns**: `{status, slug, name, unit, count, observations:
  [{ref_date, month_value, media_no_ano, media_12m}, ...]}`.
...
Search `JUROS_CATALOG` by slug / name / description fragment
...
in-memory `JUROS_CATALOG` if the DB has not been synced yet.
...
- Fetcher uses a `Semaphore(5)` to cap in-flight HTTP requests (mirrors the
  `bcb/sgs` + `bcb/focus` + `ddm/inflation` pattern).
