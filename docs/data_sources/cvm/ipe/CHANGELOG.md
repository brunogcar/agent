...
|---------|------|---------|
| v1.1 | 2026-08-04 | **Verbose sync progress output.** `sync()` now accepts `verbose=True` (default). Prints download size/time, per-year parse progress, and total elapsed to stderr (MCP-safe). IPE already used `cnpj_digits()` at ingest since v1.0 — no CNPJ fix needed. |
...
*Last updated: 2026-08-04 (v1.1 — verbose sync progress).*
