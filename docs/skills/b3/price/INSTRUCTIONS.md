...
6. **Never load `chartjs-chart-financial` for candlesticks** — The Cotação tab's candlestick is rendered by the vanilla `_renderOHLCChart` helper (flagged via `chart_data._ohlc = True`), NOT the `chartjs-chart-financial` plugin. The plugin was removed in v1.1 (it forced a `time` x-scale that required a date adapter + rendered blank without it). The `_renderOHLCChart` helper lives in `templates/js/dashboard_charts.html` (included by `dashboard.html` via `{% include %}` — extracted in Phase 3 C3). Chart sections emit `type: "chart"` with `_ohlc: True` + `_ohlc_data: [{o,h,l,c}, ...]` — never `type: "candlestick"`.
