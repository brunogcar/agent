...
|---------|------|---------|
| **v1.21** | 2026-08-07 | **WACC + DuPont + Altman Z-Score wired to dashboard.** 3 new advanced valuation metrics added to `_METRIC_DEFS` (total KPIs: 17 → 20). WACC in Market Risk tab, DuPont ROE in Profitability tab, Altman Z in Leverage tab. Tooltips added. Uses the new calculations v2.0 metrics. Suggested by external LLM review (Claude 1, Claude 2, Qwen). |
| **v1.20** | 2026-08-06 | **Ratio Grid double-multiplication fix.** `build_ratio_grid_section` in `report.py` was pre-scaling pct metrics by 100 via `_scaled(cur, 100)` AND then passing `"pct"` format to `_fmt`, which multiplied by 100 AGAIN → values showed as 2647,00% instead of 26,47%. Fixed by passing the raw fraction directly to `_fmt` with `"pct"` spec (single scaling, matching `build_overview_kpis` pattern). Found by user reviewing the PETR4 dashboard. |
| **v1.19** | 2026-08-06 | **Beta display + percentage precision + F7 cache + performance (collective LLM review).** Fixes from the code review of commits e8f8962 + e7763c2: (1) **Beta now displays** — `beta_periods` had a 1-arg signature `(company)` but the MetricSpec `history_fn` contract requires `(company, date_from, date_to)`. `summary(metric="beta")` → `TypeError` → Beta showed "—". Fixed signature + added date filtering. (2) **Percentage precision** — `summary.py` was doing `round(x, 2)` on raw fractions (0.0269 → 0.03), then `report.py` ×100 → "3,00%" (lost .69%). Fixed to `round(x, 4)`. (3) **F7 cache in ThreadPoolExecutor workers** — ContextVar doesn't propagate to worker threads, so all 17 parallel summaries ran cache-less (F7 hits=0 in workers). Each worker now enters its own `engine_cache_scope()`. (4) **`fetch_quartiles` + `fetch_series` deduplication** — was calling `history_fn` TWICE per metric (34 sequential calls). Now fetches series once + computes quartiles in-memory. Also parallelized. (5) **`if avg_1y else None` → `if avg_1y is not None else None`** — old form treated 0.0 as falsy. |
| **v1.18** | 2026-08-06 | **Growth + Beta + COE dashboard fixes (finetune v3 + v4 + v5).** (1) **3M growth = 0.0 bug fixed** — `growth_at()` anchored the prior-period search on `target_date`; when target was far beyond the latest period, the gap window included the current period → prior == curr → growth = 0.0. Fixed by anchoring on the current period's date. (2) **Beta now displays** — was only registered as an EngineSpec, not a MetricSpec, so `summary(metric="beta")` raised ValueError → no data shown. Added `register_metric(MetricSpec(name="beta", allow_negative=True))`. (3) **`allow_negative` on MetricSpec** — growth metrics (declining revenue is meaningful) + beta (countercyclical) now accept negative values in `summary()` + `compute_quartiles()`. Default False preserves valuation-ratio behavior. (4) **Tooltips added** for Cresc. Receita/Lucro, COE, Beta across all 4 tabs (Ratio Grid, Percentile Analysis, Efficiency & Growth, Market Risk). (5) **Sync guard** — `sgs` added to `REQUIRED_SOURCES` to force BCB SGS sync when historical skill is used (needed for COE/Beta). (6) **Parallel metric fetching** — 17 summary() calls now use ThreadPoolExecutor(max_workers=5). (7) **Market Risk tab** (COE + Beta) added. 8 tabs, 17 KPIs total. |
| **v1.17** | 2026-08-06 | **COE + Beta wiring + BCB range selector + parallel metrics.** Wired COE (CAPM) + Beta (5Y) into historical dashboard as Market Risk tab. Added BCB SGS to REQUIRED_SOURCES. Parallelized summary fetching via ThreadPoolExecutor. Added `price_range_selector` to BCB charts. |
| **v1.16** | 2026-08-05 | **Leverage + Efficiency + Growth tabs.** Added 3 new metric categories to dashboard: leverage (D/E, Net Debt/EBITDA, Interest Coverage), efficiency (Asset Turnover, Inventory Turnover), growth (Revenue Growth 3M, Net Income Growth 3M). Split tables in Ratio Grid. |
| **v1.15** | 2026-08-05 | **Dashboard v3 pattern + tooltips + trend line charts.** Applied the financials/valuation dashboard pattern: company header, price chart, split tables, 5Y averages, tooltips on all ratio_grid cells, trend line charts per metric. |
| **v1.14** | 2026-07-31 | **Dashboard reorg + F7 speed fix + sync guard.** Dashboard reorganized from 3 tabs to 5 tabs: Overview (8 KPIs — added Marg. Bruta + Marg. Líquida), Valuation (subtabs: Percentile table + chart + Trend), Profitability (subtabs: Percentile table + chart + Trend), Ratio Grid, Percentile Analysis (full table + chart). **Speed fix:** all 8 summary() + fetch_quartiles() calls now wrapped in `with engine_cache_scope():` so F7 caches shared engines across metric calls (was slow because F7 only activated inside compute_all_ratios which historical doesn't use). Added detailed `[historical]` print output. Sync guard wired via `required_sources=["dfp", "itr", "cotahist", "bridge"]`. ROADMAP.md created. 8 tests (was 5). |
| **v1.13** | 2026-07-30 | **skills/_base/ extraction.** _registry.py + __init__.py now delegate to the shared `skills/_base/` module (ModeSpec + make_registry + make_route + auto_discover_modes). _registry.py shrank from ~97 lines to ~16 lines; __init__.py shrank from ~88 lines to ~50 lines. No behavior change — same modes, same route() signature, same MANIFEST. Bug fixes to the dispatch infrastructure now only need to be made in ONE place (skills/_base/) instead of 11. Historical's `_auto_register_metric_history_modes()` (which auto-registers `<metric>_history` modes from calculations METRICS) was PRESERVED — see [ARCHITECTURE.md](ARCHITECTURE.md). |
| **v1.12** | 2026-07-29 | **Modular split + dashboard mode.** Split monolithic `historical.py` (253 lines) into `_registry.py` + `modes/` (3 explicit mode files: `ratio_history`, `summary`, `dashboard`) + `helpers.py` + `report.py` — mirrors the `financials`/`valuation`/`comparison`/`backtest`/`dividends`/`governance` modular pattern (`_registry.py` + `modes/*.py` auto-discovered via `@register_mode`). New `dashboard` mode produces a multi-tab dashboard payload for the report tool. New `historical_dashboard` adapter (report tool v1.7, 71st adapter). Historical now exposes 40 modes total (37 auto-generated `<metric>_history` + 3 explicit: `ratio_history`, `summary`, `dashboard`). The metric/engine library still lives in `calculations/` (unchanged from v1.11). |
...
| v1.1 | 2026-07-26 | PL engine + VPA metric + engine/metric separation. New `engines/bpp/pl.py` (Patrimônio Líquido snapshot from DFP+ITR BPP 2.03). New `metrics/vpa.py` (P/VPA = price / (PL / shares)). New `vpa_history` mode. `summary` + `ratio_history` made metric-aware. Documented engine vs metric pattern. |
