...
|---------|------|---------|
| **v1.16** | 2026-08-05 | **Leverage + Efficiency + Growth tabs.** Added 7 new metrics to the dashboard: `debt_equity`, `net_debt_ebitda`, `interest_coverage` (Leverage), `asset_turnover`, `inventory_turnover` (Efficiency), `revenue_growth_3m`, `net_income_growth_3m` (Growth). Dashboard reorganized from 5 tabs to 7 tabs: added "Liquidez e Alavancagem" + "Eficiência e Crescimento" tabs, each with the standard subtab layout (Percentil, Gráfico, Tendência) + per-metric 5Y line charts. The existing `build_trend_line_chart` helper (added in v1.14 but only applied to Valuation + Profitability) is now applied to all metric categories — closes ROADMAP H1. All 7 metrics already existed in `calculations` (debt_equity + net_debt_ebitda + asset_turnover since v1.10, interest_coverage + inventory_turnover since v1.3, growth metrics since v1.7) — this version surfaces them in the historical dashboard rather than adding new computation. Closes ROADMAP H2 (leverage/efficiency/growth). H4 (Beta 5Y) closed as Done — built in calculations v1.13, not historical. H5 (BCB SGS macro indicators) closed as Done — BCB SGS built v1.0. |
