...
|---------|------|---------|
| **v1.3.1** | 2026-08-27 | **Dashboard empty-tickers short-circuit (test perf).** `modes/dashboard.py` now returns the validation error `{"status": "error", "error": "tickers (list) is required. Min 2."}` IMMEDIATELY when `tickers` is empty — BEFORE calling `build_company_header` / `build_price_chart` (which hit real DBs + brapi). This was the root cause of the 3s+ `test_route_dashboard_dispatches` test: the validation-error test case `route(mode="dashboard")` with no tickers was falling through to the header/chart builders, which made real brapi HTTP calls + opened SQLite DBs before the validation logic downstream caught the empty list. No API change — the error message + status are identical to what the downstream validation already returned; the short-circuit just prevents the wasted I/O. Part of the cross-cutting test-optimization pass (full suite 201s → ~130s). |
| **v1.3** | 2026-08-05 | **brapi concurrency for multi-ticker fetching (Phase 3 perf).** `fetchers.py` + `modes/growth.py` now wrap their per-ticker fetch loops in a `concurrent.futures.ThreadPoolExecutor(max_workers=5)` — tickers are fetched concurrently instead of sequentially. Same pattern as the screener skill's `sector` mode (see [screener CHANGELOG](../screener/CHANGELOG.md) v1.3): both modes hit the brapi quote endpoint per ticker via `valuation.ratios()`, so the wall-clock savings scale with the number of tickers in the comparison. For a typical 5-10 ticker comparison the speedup is ~3-5× (limited by the `threading.Semaphore(5)` inside `data_sources/b3/brapi/fetcher.py` — see [brapi CHANGELOG](../../../data_sources/b3/brapi/CHANGELOG.md) v1.1). Removed the `engine_cache_scope` ContextVar wrapper from the per-ticker call path — `ContextVar` is **thread-local by design**, so the cache wasn't shared across worker threads anyway (each worker got its own empty cache). The v1.9 F7 cache still applies within a single worker's `ratios()` call (engines shared across metrics inside one ticker). No API changes — the `side_by_side()`, `summary()`, and `growth()` result shapes are unchanged; only the internal fetch loop is parallelized. No new tests (the existing comparison tests mock `valuation.ratios` + `financials.summary` so the threading is transparent to assertions). |
...
*Last updated: 2026-08-05 (v1.3 — brapi concurrency for multi-ticker fetching).*
