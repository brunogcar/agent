...
|---------|------|---------|
| **v1.2** | 2026-07-31 | **Dashboard reorg + sync guard.** Added 2 chart builders to `report.py`: `build_shareholder_doughnut` (doughnut chart of top shareholder distribution, on Top Shareholders tab) + `build_equity_structure_bar` (bar chart of BPP 2.03.* equity structure components, on Equity Structure tab). Wired sync guard via `required_sources=["fre","dfp","bridge"]` + `make_route()`. Added detailed `print("[shareholders] ...")` progress output (flush=True) showing starting message, data fetching, building sections, done message with tab count. Created `ROADMAP.md` with 4 backlog items (concentration index, multi-period changes, insider overlay, treasury shares). 7 tests (was 6). |
