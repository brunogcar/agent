...
#### v2.4 — Collapsible macro dropped nested `sections`
> - **What happened:** `build_red_flags_section` in `analysis.py` (since v2.0) returns `{"type": "collapsible", "title": ..., "open": ..., "sections": [<table section>]}` — the consistency-checks table is nested in `sec.sections`. But the `collapsible` macro in `macros.html` only rendered `{{ content }}` (the `sec.text` field, used for the empty-state message) and never `sec.sections`. The Red Flags body was always empty — the section title rendered (via the generic `sec.title` → `<div class="section-title">` path), but the actual checks table was silently dropped.
> - **Why it matters:** The bug was latent for 4 months (v2.0 → v2.4). It went unnoticed because Red Flags sat at the bottom of a long Overview tab — the empty collapsible looked like a "no flags" state. The v2.3 Overview subtab split moved Red Flags into the "Análise de Risco" subtab where the empty body was prominent, and the user noticed it immediately.
> - **Fix:** Added a `sections` parameter to the `collapsible` macro that renders nested sections via `_section_inner`. Both call sites (`dashboard.html` top-level + `macros.html` subtabs) pass `sections=sec.get('sections')` + a unique `prefix` for canvas IDs (so future charts inside a collapsible render correctly). Added chart-render loops for collapsible nested sections in `dashboard.html` (top-level + subtab).
> - **Lesson:** When a section type carries nested data (`sections`, `tabs`, `annual_sections`/`quarterly_sections`), the rendering macro MUST iterate that nested data — not just the flat fields. A section type with `sections=[...]` that's rendered by a macro ignoring `sections` is a silent data-loss bug. Grep for `sec.type == 'X'` in the templates + verify the corresponding macro reads ALL the fields the builder emits.

#### v2.4 — Quarterly Trend delta direction (off-by-one + inverted)
> - **What happened:** The Overview "Quarterly Trend" table added Δ% columns in v2.3 (a QoQ % change per metric). The loop iterated `reversed(quarterly_periods)` (newest-first) with `prev_metrics = m` set at the END of the loop. So on the next iteration, `prev_metrics` was the NEWER period (the one just rendered above), not the OLDER period (the one before in time). The deltas were computed as `(current − newer) / newer` — inverted in sign AND shifted by one row.
> - **Why it matters:** The newest row showed `—` (no `prev_metrics` yet on the first iteration), but it SHOULD show the delta vs the 2nd newest. The oldest row showed a delta, but it SHOULD show `—` (no older period to compare to). A user comparing quarter-over-quarter growth saw the wrong sign + the wrong row alignment.
> - **Fix:** Pre-sort newest-first via an explicit chronological key (robust to either input order — the original `reversed()` assumed oldest-first input which is fragile). Then for row at index `i`, the "previous in time" is at index `i+1` (older). The oldest row (last) has no older period → `—`. The newest row (first) shows the delta vs the 2nd newest (correct QoQ).
> - **Lesson:** When computing period-over-period deltas in a display loop, the "previous" period is the one BEFORE in TIME, not the one before in the ITERATION. If you iterate newest-first, `prev` is at `i+1` (older), not `i-1` (newer, already rendered). Always name the variable clearly (`prev_in_time` vs `prev_rendered`) + add a comment explaining the direction.

---

*Last updated: 2026-09-04 (v2.4 — collapsible-nested-sections + delta-direction lessons; v2.0 report-split-pattern lesson). See [CHANGELOG.md](CHANGELOG.md) for version history.*
