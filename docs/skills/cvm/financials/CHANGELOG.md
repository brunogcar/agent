...
|---------|------|---------|
| **v1.12** | 2026-07-29 | **Dashboard reorg + 5 standalone statement modes + generic adapter.** `dashboard` mode reorganized from 5 tabs to 7 tabs (Overview / Indicadores / Crescimento / Balanço / DRE / DFC / DVA). New section types: `subtabs` (BPA + BPP under Balanço), `chart` (growth bar / margin trend line / FCO/FCI/FCF stacked bar / DVA doughnut), `ratio_grid` (Indicadores with all 7 ratio categories), `collapsible`. KPI cards relabeled: Receita (TTM), EBITDA, Lucro Líquido, ROE, ROIC, Dív.Líq/EBITDA. New 5 standalone statement modes (bpa, bpp, dre, dfc, dva) — thin wrappers over `complete(grupo=...)` that reshape the per-period `accounts` list into a `{codigo: {label, section, valor_brl}}` dict, with a `section` field derived from the codigo prefix (Ativo Circulante / Geração / Distribuição, etc.). New shared helper `modes/_statement_sections.py` with the section classifiers + reshape helper. New generic `financials_statement` adapter (consumes any of the 5 standalone modes; groups by `section`; columns [Código, Descrição, Valor (BRL)]; pre-formats via `brl` spec). Dashboard calls the 5 standalone modes via try/except — a failure in one statement degrades the corresponding tab to an error text section instead of crashing the whole dashboard. `test_dashboard.py` updated for 7-tab assertions; `FINANCIALS_DASHBOARD` synthetic test data refreshed for typed sections. `test_route.py` updated for 7-tab name list. `test_report_adapters.py` adds `TestFinancialsStatementAdapter` (6 tests covering DRE grouping, DVA Geração/Distribuição split, KPI strip, error + empty-periods + empty-accounts paths). 290 financials + adapter tests pass with `-W error`. |
