...
|---------|------|---------|
| **v1.2** | 2026-07-31 | **Dashboard reorg + sync guard.** Added drawdown chart (underwater equity curve) to Performance tab — shows how far below peak the equity is at each point, as a filled area chart. Sync guard wired via `required_sources=["cotahist", "bridge"]` + `make_route()`. ROADMAP.md created. 10 tests (was 9). |
| **v1.3** | 2026-07-30 | **skills/_base/ extraction.** _registry.py + __init__.py now delegate to the shared `skills/_base/` module (ModeSpec + make_registry + make_route + auto_discover_modes). _registry.py shrank from ~97 lines to ~16 lines; __init__.py shrank from ~88 lines to ~50 lines. No behavior change — same modes, same route() signature, same MANIFEST. Bug fixes to the dispatch infrastructure now only need to be made in ONE place (skills/_base/) instead of 11. |
