...
| `skills/cvm/governance/__init__.py` | [v2.0] Uses `auto_discover_modes()` + `make_route()` from `skills/_base/` — ~50 lines. MANIFEST + route — modes auto-generated from `_registry.py`. |
| `skills/cvm/governance/_registry.py` | [v2.0] Delegates to `skills/_base/` — creates skill's own MODES dict via `make_registry()`. ~16 lines. |
