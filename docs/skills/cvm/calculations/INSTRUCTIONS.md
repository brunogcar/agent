...
14. **Never forget `@engine_cached` on a new engine** (v1.8 lesson) — every engine's `at_fn` + `periods_fn` MUST be decorated with `@engine_cached` (from `skills._base`). Without it, the engine is queried N times per `compute_all_ratios()` call instead of once (one per metric that composes it). The decorator is a passthrough when no cache scope is active (zero overhead for standalone calls), so there's no reason to skip it. The decorator MUST be applied at definition time (not in `register_engine()`) — metrics use `from engines.X import Y` which binds a direct reference at import time; monkey-patching the module attribute later is invisible to metrics.
...
11. **Always decorate engine fns with `@engine_cached`** (v1.8) — `from skills._base import engine_cached` then `@engine_cached` on both `<name>_at` and `<name>_periods`. The decorator is a passthrough when no cache scope is active (zero overhead for standalone calls). Inside `compute_all_ratios()`, the cache activates and shared engines are queried ONCE per (company, date) instead of N times. See NEVER DO #14.
