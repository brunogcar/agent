...
|---------|------|---------|
| **v1.9** | 2026-07-31 | **F7 engine cache (ContextVar-scoped per-call cache).** Added `@engine_cached` decorator + `engine_cache_scope` context manager to `skills/_base.py`. The decorator is applied to all 34 engines' `at_fn` + `periods_fn` at module definition time. Inside `compute_all_ratios()`, the loop is wrapped in `with engine_cache_scope():` — engines shared across metrics (`earnings` used by 11 metrics, `pl` by 10, `debt` by 10, `revenue` by 15) are queried ONCE per `(company, date)` instead of N times. ~60% fewer DB queries per dashboard call. **Why decorator (not monkey-patch):** Metrics use `from engines.X import Y` — a direct reference bound at import time. Monkey-patching the module attribute after import is invisible to metrics. The decorator is applied at definition time, so `spec.at_fn` and `module.fn` are the same wrapper object, permanently — all 14 registry identity tests pass unchanged. **Thread safety:** ContextVar is per-thread + per-asyncio-task. No lock needed. **Reentrancy:** Nested `compute_all_ratios()` calls reuse the outer scope's cache (inner `engine_cache_scope.__enter__` detects active scope + doesn't install a new one). **Zero overhead when inactive:** When no scope is active (standalone engine calls, tests), the decorator is a passthrough. NEW `tests/skills/cvm/calculations/test_perf_infra.py` (17 tests): verifies caching actually works (DB query count drops, not just function identity), reentrancy, None-value caching, periods_fn caching, real engine identity. 34 engine files modified (1 import + 2 `@engine_cached` decorators each). `skills/cvm/calculations/_registry.py` `compute_all_ratios()` wraps loop in scope. `skills/_base.py` adds `engine_cached` + `engine_cache_scope` + `_ENGINE_CACHE` ContextVar. 1155 tests pass (1138 existing + 17 new). Zero test changes outside the new test file. |
