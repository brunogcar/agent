...
|---------|------|---------|
| **v1.10** | 2026-07-31 | **F8 materialized ratios (pre-computed fundamental metrics).** NEW `skills/cvm/calculations/_materialized.py` with SQLite table `ratios_materialized(ticker, date, metric_name, value, computed_at)` in `memory_db/cvm/ratios.db`. Normalized (long) format — one row per (ticker, date, metric), index on (ticker, date). Only STABLE FUNDAMENTAL categories materialized (profitability, liquidity, leverage, efficiency, tax = ~20 metrics). Growth NOT materialized (lookback-dependent — value depends on when evaluated, risks the staleness problem fixed in v1.8 gap-tolerance). Valuation + per_share NOT materialized (price-dependent — depend on daily COTAHIST, would go stale in hours). `compute_all_ratios()` reads materialized first for fundamental metrics; only price-based + growth + stale/missing computed live. Turns a 35-engine call into ~15 live + 1 DB lookup. **Event-driven invalidation:** `ensure_fresh()` calls `materialize_ratios(company, today)` after successful force-sync — no time-based staleness window (per LLM review consensus: don't build two independent staleness clocks). **Thread-local connection:** `_get_ratios_conn()` uses `threading.local()` + WAL journal mode (Qwen P1). **Escape hatch:** `CVM_SKIP_MATERIALIZED=1` env var. 8 new tests (materialize+read, growth not materialized, valuation not materialized, missing→None, env skip, clear, stats, compute_all_ratios integration). 1177 tests pass (1169 existing + 8 new). |
