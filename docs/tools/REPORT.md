...
| `mermaid_src \| safe` in diagram template | Auto-escaped (no `\| safe`) | Mermaid.js parses escaped text correctly |
...
- Added `_atomic_write` to `render_template` (temp file + `os.replace`)
...
6. **Never use `| safe` in templates for user-controlled text** — XSS vector. Jinja2 autoescape handles it.
7. **Never touch `@meta_tool` or `@register_action` shared decorators** — use `help_text` for param docs.
...
14. **Never add `force` requirement to non-destructive actions** — only `export` (overwrites PDF/PNG) needs consideration.
15. **Keep tool facade thin** — validate, preset merge, dispatch, wrap, memory hook. Business logic in builders.
16. **Document design decisions in comments** — explain WHY, not just WHAT. Other AIs audit the code.
