...
|---------|------|---------|
| **v1.11** | 2026-08-02 | **Investsite dashboard adapter v2.0 + template tooltip fix.** (1) `investsite_dashboard.py` adapter — preserves `tab["group"]` field, passes `company_header` + `freshness_footer` through. (2) `macros.html` — `data_table` macro now supports dict cells `{"text": label, "tooltip": formula}` for hover tooltips on table rows. (3) `base.html` — CSS tooltip selector extended to `td[data-tooltip]:hover::after` (was only `.ratio-item`). Added `td[data-tooltip] { position: relative; }` for correct tooltip anchoring. Aligns with investsite v2.0 (12-tab dashboard with 4 sidebar groups). |
| **v1.10** | 2026-08-02 | **Dashboard template v3 — company_info section + price_range_selector + freshness_footer + custom CSS tooltips.** Major template enhancements mirroring the financials + valuation dashboard overhauls: (1) `dashboard.html` — new `company_info` section type renders a company header card (FCA/CAD/COTAHIST info: name, CNPJ, CD_CVM, ISIN, sector, listing segment, control type, UF, website, fiscal year-end, last close). (2) `dashboard.html` — new `price_range_selector` field on chart sections renders Tudo/5A/1A/1M buttons that filter the price chart client-side via `filterPriceChart()` JS. (3) `dashboard.html` — `freshness_footer` variable rendered as a compact bar at the bottom of the content area. (4) `dashboard.html` + `macros.html` — `sec.description` now rendered as a text-block above each chart. (5) `base.html` — custom CSS tooltip via `.ratio-item[data-tooltip]:hover::after` pseudo-element (native `title=` tooltips unreliable on non-link elements). (6) `base.html` — `.ratio-info` icon (ⓘ) styling with hover effect. (7) `macros.html` — `ratio_grid` macro updated to render `title=` + `data-tooltip=` + ⓘ icon when `item.tooltip` is present. (8) `macros.html` — `subtabs` macro updated to render `sec.description`. (9) `html.py` `build_dashboard()` — now forwards `company_header` + `freshness_footer` from adapter output to the template context. (10) `financials_dashboard.py` adapter — preserves `tab["group"]` field for sidebar grouping + passes `company_header` + `freshness_footer` through. (11) `valuation_dashboard.py` adapter — pass-through already works (dashboard mode returns typed sections). Aligns with financials v1.16.1 + valuation v1.8. |
| **v1.9** | 2026-07-29 | **Subtabs support + generic statement adapter + dashboard centering.** (1) `base.html` template: body is now centered (`margin-right: auto` on `.main`) so dashboards render in a readable column on wide screens. Added `.subtabs-bar` / `.subtab-btn` / `.subtab-panel` CSS + `showSubTab()` JS for nested tab navigation within a dashboard tab. (2) `macros.html`: new `subtabs(tabs_list, prefix)` macro that renders a sub-tab bar + panels; each sub-tab carries its own `sections` list with the same section-type dispatch as the parent dashboard. (3) `dashboard.html`: dispatches `type: "subtabs"` sections to the new macro; chart canvases inside sub-tabs get unique IDs via the `prefix` arg. (4) New `financials_statement` adapter (consumes any of the 5 standalone statement modes from financials v1.12: bpa / bpp / dre / dfc / dva; groups by `section` field — Ativo Circulante / Geração / Distribuição, etc.; columns [Código, Descrição, Valor (BRL)]; pre-formats via `brl` spec). (5) `financials_dashboard` adapter: `_format_kpi` now passes through pre-formatted string KPI values verbatim (the v1.12 dashboard pre-formats via `apply_fmt` in `skills/cvm/financials/report.py`); raw numeric KPI values still get formatted via the unit→spec map. Total 76 adapters (was 75). Aligns with financials v1.12 (5 standalone statement modes + 7-tab dashboard reorg). |
| v1.8 | 2026-07-29 | **4 new dashboard adapters (screener + shareholders + insider + investsite).** Added `screener_dashboard` (consumes `screener.dashboard()` — Overview KPIs + Peers + Comparison; thin pass-through), `shareholders_dashboard` (consumes `shareholders.dashboard()` — Overview + Top Shareholders + Free Float + Equity Structure; thin pass-through), `insider_dashboard` (consumes `insider.dashboard()` — Overview + Recent Transactions + By Role + Monthly Net; thin pass-through), and `investsite_dashboard` (consumes `investsite.dashboard()` — Overview + Key Indicators + Latest Events; thin pass-through). All four follow the v1.5-v1.7 dashboard-adapter pattern. Total 75 adapters (was 71). Aligns with the screener v1.4 + shareholders v1.1 + insider v1.1 + investsite v1.1 skill splits (monolithic `.py` → `modes/ + _registry.py` modular structure). With this, ALL 11 skills (10 CVM + investsite) now use the modular pattern. |
| v1.7 | 2026-07-29 | **3 new dashboard adapters (dividends + governance + historical).** Added `dividends_dashboard` adapter (consumes `dividends.dashboard()` multi-tab result — Overview + Events + Annual + Payable + Filings; thin pass-through), `governance_dashboard` adapter (consumes `governance.dashboard()` — Overview KPIs + Practices + By Chapter; thin pass-through), and `historical_dashboard` adapter (consumes `historical.dashboard()` — Overview + Ratios + Summary; thin pass-through). All three follow the v1.5/v1.6 dashboard-adapter pattern (thin adapters — the skill's `dashboard()` mode already produces typed sections). Total 71 adapters (was 68). Aligns with the dividends v1.1 + governance v1.1 + historical v1.2 skill splits (monolithic `.py` → `modes/ + _registry.py` modular structure). |
| v1.6 | 2026-07-29 | **2 new dashboard adapters (backtest + comparison).** Added `backtest_dashboard` adapter (consumes `backtest.dashboard()` 3-tab result: Overview with KPIs + equity curve, Trades table, Performance summary — thin pass-through of the tab payload) and `comparison_dashboard` adapter (consumes `comparison.dashboard()` 5-tab result: Overview/Valuation/Financials/Dividends/Growth — thin pass-through). Both follow the v1.5 dashboard-adapter pattern (thin adapters — the skill's `dashboard()` mode already produces typed sections). Total 68 adapters (was 66). Aligns with the backtest v1.1 + comparison v1.5 skill splits (monolithic `.py` → `modes/ + _registry.py` modular structure). |
...
*Last updated: 2026-07-29 (v1.7). See [ARCHITECTURE.md](ARCHITECTURE.md) for file maps, [API.md](API.md) for action details, [INSTRUCTIONS.md](INSTRUCTIONS.md) for AI editing rules.*
