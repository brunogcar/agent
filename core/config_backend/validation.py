"""core/config_backend/validation.py — Startup config validation (fail-fast).

[v1.0] Merged from ``core/config_validation.py`` as part of the config_backend
split. The original module is now a 3-line shim that re-exports
``validate_config`` from here.

DIFFERENT from ``validators.py``:
    - validators.py :: _validate_config(cfg)  — inline range checks during
      Config.__init__ (construction-time, raises immediately).
    - validation.py :: validate_config()      — startup checks (called by
      server.py after all imports done). Aggregates ALL errors then raises
      a single RuntimeError with a multi-line summary, so the operator sees
      every problem in one shot rather than fixing one error per boot.

Checks performed here (all aggregated into one RuntimeError):
    - Critical paths exist (agent_root, workspace_root, memory_root, ...)
    - planner_model / executor_model / router_model are non-empty
    - All timeouts > 0 (sandbox, execution, planner, router, autocode_graph)
    - autocode_max_retries / autocode_max_file_chars > 0
    - lm_studio_base_url starts with http:// or https://
    - Every model_registry entry has non-empty model/provider + positive timeout
    - Every ROLES[*].llm_role is present in model_registry (typo guard,
      opt-in roles like consultor are skipped)
    - allowed_internal_hosts is a frozenset/set/list of non-empty strings
"""
...
def validate_config() -> None:
    ...
    # after splitting). The frozenset comprehension in security.py filters empty
