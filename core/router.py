...
class TaskRouter:
    ...
    def route(
        ...
        # [v1.1 #18] Swarm fallback for low-confidence routing
        if decision.confidence == "low" and getattr(cfg, "router_swarm_fallback", False):
            swarm_decision = self._swarm_fallback_route(goal, trace_id)
            if swarm_decision:
                if trace_id:
                    tracer.step(trace_id, "router",
    ...
    def _swarm_fallback_route(self, goal: str, trace_id: str) -> Optional[RoutingDecision]:
        """[v1.1 #18] Ask swarm to vote on workflow type when heuristics are uncertain.

        Called from route() ONLY when:
          - the Router model was unavailable (so _model_route returned None), AND
        ...
        the heuristic decision. The router_swarm_fallback flag is advisory —
        it must never turn a successful route() call into a failed one.
        """
        try:
