"""
patch_cvm_dfp_itr.py
Deploy to: D:\mcp\agent\patch_cvm_dfp_itr.py
Run:       python patch_cvm_dfp_itr.py

Patches skills/cvm/cvm_dfp_itr/cvm_dfp_itr.py to:
  1. Replace inline _connect() with import from skills.cvm._db
  2. Add bridge ticker lookup at top of _resolve_company()

WHAT CHANGES:
  _connect()          -> wraps _db.connect_dfp_itr() (same behavior, shared code)
...
WHAT DOES NOT CHANGE:
  - CVM_DB_PATH import from cvm_dfp_itr_catalog (still used in status mode)
...
DECISION: _resolve_company returns list[dict] in cvm_dfp_itr (different from
...
TARGET = Path(__file__).parent / "skills" / "cvm" / "cvm_dfp_itr" / "cvm_dfp_itr.py"

# ── Patch 1: replace _connect() with shared _db wrapper ──────────────────────

OLD_CONNECT = '''def _connect() -> sqlite3.Connection:
    """
    Open dfp_itr.db as read-only.
    ...
    dfp_itr.db is updated externally by dfp_itr_sync -- this skill never writes.
    """
    if not CVM_DB_PATH.exists():
        raise FileNotFoundError(
            f"dfp_itr.db not found at {CVM_DB_PATH}. "
            f"Move it there: mkdir {CVM_DB_PATH.parent} && copy dfp_itr.db {CVM_DB_PATH}"
        )
...
def main():
    ...
    if not failed:
        print("Done. cvm_dfp_itr.py patched successfully.")
        print("Test with: skill(domain='cvm_dfp_itr', mode='query', params='{\"company\":\"PETR4\"}')")
    else:
