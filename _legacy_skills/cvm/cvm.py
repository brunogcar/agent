...
Planned sub-domains:
  cvm_fre       -- FRE filings: dividends per share, shareholders, compensation
  cvm_itr       -- Direct ITR quarterly filing queries
"""
...
def _discover_sub_domains() -> dict:
    """Scan skills/cvm/ for sub-domain packages. Cached after first call."""
    global _SUB_DOMAINS
    if _SUB_DOMAINS is not None:
        return _SUB_DOMAINS

    _SUB_DOMAINS = {}
    cvm_dir = Path(__file__).resolve().parent

    for item in sorted(cvm_dir.iterdir()):
...
def route(sub_domain: str = "", mode: str = "", **kwargs) -> dict:
    ...
    if not sub_domains:
        return {"status": "error", "error": "No cvm sub-domains found in skills/cvm/"}
