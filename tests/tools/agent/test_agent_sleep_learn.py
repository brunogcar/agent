...
class TestAgentSleepLearnIntegration:
    ...
    def test_inject_rules_called_for_high_latency_role(self, mock_llm_result):
        ...
        with patch("tools.agent.llm.complete", return_value=mock_llm_result) as mock_llm, \
             patch("tools.agent._inject_rules", return_value="INJECTED PROMPT") as mock_inject:
    ...
    def test_inject_rules_not_called_for_router_roles(self, mock_llm_result):
        ...
        with patch("tools.agent.llm.complete", return_value=mock_llm_result) as mock_llm, \
             patch("tools.agent._inject_rules", return_value="INJECTED") as mock_inject:
    ...
    def test_inject_rules_failure_falls_back_to_original_prompt(self, mock_llm_result):
        ...
        with patch("tools.agent.llm.complete", return_value=mock_llm_result) as mock_llm, \
             patch("tools.agent._inject_rules", side_effect=RuntimeError("ChromaDB down")) as mock_inject:
    ...
    def test_inject_rules_respects_trace_id(self, mock_llm_result):
        ...
        with patch("tools.agent.llm.complete", return_value=mock_llm_result), \
             patch("tools.agent._inject_rules", return_value="PROMPT") as mock_inject:
    ...
    def test_inject_rules_skipped_when_module_unavailable(self, mock_llm_result):
        ...
        with patch("tools.agent.llm.complete", return_value=mock_llm_result) as mock_llm, \
             patch("tools.agent._inject_rules", None):
