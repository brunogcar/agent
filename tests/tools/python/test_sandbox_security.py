...
class TestSandboxSecurity:
    ...
    def test_safe_math_execution(self):
        ...
        result = python(action="run", code="x = 2 + 2\nprint(x)")
    ...
    def test_blocks_open_call(self):
        ...
        result = python(action="run", code="f = open('test.txt', 'w')")
        assert result["status"] == "error"
    ...
    def test_blocks_dunder_import(self):
        ...
        result = python(action="run", code="os = __import__('os')\nprint(os.name)")
        assert result["status"] == "error"
    ...
    def test_syntax_error_handling(self):
        ...
        result = python(action="run", code="print('hello'")
        assert result["status"] == "error"
        assert "SyntaxError" in result["error"]

    def test_output_capture(self):
        """Only print() output should be captured, not variable assignments."""
        result = python(action="run", code="x = 10\ny = 20\nprint(x + y)")
        assert result["status"] == "success"
