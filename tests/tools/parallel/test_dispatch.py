"""Tests for parallel facade dispatch and registry.

Mirrors the structure of tests/tools/consult/test_dispatch.py. Covers:
  - Unknown action → error listing valid actions
  - Empty action → error explaining action is required
  - Case insensitivity (RUN / Run / run all dispatch the same handler)
  - All 3 actions present in DISPATCH with full metadata
  - @meta_tool generated Literal["run", "race", "pipeline"] on the facade
  - Facade-generated docstring includes the action list
"""
from __future__ import annotations

from tools.parallel import parallel


class TestDispatch:
    """Dispatcher routes actions and handles unknown/empty actions."""

    def test_unknown_action(self):
        result = parallel(action="nonexistent")
        assert result["status"] == "error"
        assert "Unknown action" in result["error"]
        assert "run" in result["error"]
        assert "race" in result["error"]
        assert "pipeline" in result["error"]

    def test_empty_action(self):
        result = parallel(action="")
        assert result["status"] == "error"
        assert "action is required" in result["error"].lower()

    def test_action_case_insensitive(self, mock_cfg):
        from unittest.mock import MagicMock, patch
        from tools.parallel_ops.tool_map import _TOOL_MAP

        mock_fn = MagicMock(return_value={"status": "success"})
        with patch.dict(_TOOL_MAP, {"web": mock_fn}, clear=False):
            upper = parallel(action="RUN", tasks=[{"name": "web", "args": {}}])
            assert upper["status"] == "success"

            mixed = parallel(action="Run", tasks=[{"name": "web", "args": {}}])
            assert mixed["status"] == "success"

    def test_duration_ms_always_present(self, mock_cfg):
        from unittest.mock import MagicMock, patch
        from tools.parallel_ops.tool_map import _TOOL_MAP

        mock_fn = MagicMock(return_value={"status": "success"})
        with patch.dict(_TOOL_MAP, {"web": mock_fn}, clear=False):
            result = parallel(action="run", tasks=[{"name": "web", "args": {}}])
            assert "duration_ms" in result
            assert isinstance(result["duration_ms"], (int, float))
            assert result["duration_ms"] >= 0

    def test_handler_exception_caught(self):
        """If the handler raises, the facade should return a graceful error."""
        from unittest.mock import patch
        from tools.parallel_ops._registry import DISPATCH

        with patch.dict(DISPATCH, {"parallel": {"run": {"func": _raising_handler, "help": "", "examples": []}}}):
            result = parallel(action="run", tasks=[{"name": "web", "args": {}}])
            assert result["status"] == "error"
            assert "Parallel action failed" in result["error"]
            assert "boom" in result["error"]

    def test_trace_id_threaded_through_dispatch(self):
        """trace_id should be present in error responses from dispatch layer."""
        result = parallel(action="nonexistent", trace_id="trace-dispatch-1")
        assert result["status"] == "error"
        assert result["trace_id"] == "trace-dispatch-1"

        result = parallel(action="", trace_id="trace-dispatch-2")
        assert result["status"] == "error"
        assert result["trace_id"] == "trace-dispatch-2"


def _raising_handler(**kwargs):
    raise RuntimeError("boom")


class TestRegistry:
    """Verify all 3 parallel actions are registered in DISPATCH."""

    def test_dispatch_has_3_actions(self):
        from tools.parallel_ops._registry import DISPATCH
        actions = DISPATCH.get("parallel", {})
        assert len(actions) == 3
        expected = {"run", "race", "pipeline"}
        assert set(actions.keys()) == expected

    def test_all_actions_have_metadata(self):
        from tools.parallel_ops._registry import DISPATCH
        for name, info in DISPATCH["parallel"].items():
            assert "func" in info, f"{name} missing func"
            assert "help" in info, f"{name} missing help"
            assert "examples" in info, f"{name} missing examples"
            assert callable(info["func"]), f"{name} func not callable"
            assert isinstance(info["help"], str) and info["help"], f"{name} help empty"
            assert isinstance(info["examples"], list) and info["examples"], f"{name} examples empty"

    def test_action_names_match_pattern(self):
        """All action names must match ^[a-z][a-z0-9_]*$ (validated by @meta_tool)."""
        import re
        from tools.parallel_ops._registry import DISPATCH
        pattern = re.compile(r"^[a-z][a-z0-9_]*$")
        for name in DISPATCH["parallel"]:
            assert pattern.match(name), f"{name!r} does not match required pattern"

    def test_facade_action_literal_generated(self):
        """The @meta_tool decorator should have replaced `action: str` with a Literal."""
        from typing import get_args, get_type_hints
        from tools.parallel import parallel as parallel_fn
        hints = get_type_hints(parallel_fn)
        action_hint = hints.get("action")
        # Literal[...] args should be exactly {"run", "race", "pipeline"} (sorted).
        args = set(get_args(action_hint))
        assert args == {"run", "race", "pipeline"}, f"Got: {args}"

    def test_facade_docstring_has_action_list(self):
        """The @meta_tool decorator should have generated a docstring with action list."""
        from tools.parallel import parallel as parallel_fn
        assert parallel_fn.__doc__ is not None
        doc = parallel_fn.__doc__
        assert "run" in doc
        assert "race" in doc
        assert "pipeline" in doc
        assert "parallel meta-tool" in doc.lower()
