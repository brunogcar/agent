"""tests/workflows/understand/test_structure.py

[v1.4.2 SPLIT] Structural/regression tests for the understand workflow.

Moved from test_helpers.py — these are graph-level invariants, not helper-
module tests:
  - TestSyncNodes            — all 4 nodes are sync (def, not async def)
  - TestNoEventLoopHack      — facade has no ThreadPoolExecutor/new_event_loop/asyncio.gather
  - TestSubpackageStructure  — understand_impl subpackage has the correct module shape
  - TestCompletedWithErrors  — facade treats 'completed_with_errors' as success
"""
