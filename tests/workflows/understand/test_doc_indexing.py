...
class TestExtractDocChunks:
    ...
    def test_line_start_line_end_are_zero(self):
        """Doc chunks don't have reliable line numbers — always 0/0."""
        from core.kgraph.embeddings import extract_doc_chunks
        with patch("tools.file_ops.actions.read_file._chunk_text", return_value=["chunk text"]):
            chunks = extract_doc_chunks("content", "test.md", "t1")
        for c in chunks:
...
class TestParseAndStoreDocBranch:
    ...
    def test_doc_file_gets_no_graph_edges(self, mocker, tmp_path):
        ...
        mocker.patch("workflows.understand_impl.nodes.parse_and_store.ProjectManager")
