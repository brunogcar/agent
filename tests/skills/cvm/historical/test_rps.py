"""Tests for skills/cvm/historical/metrics/rps.py + engines/revenue.py.
...
import pytest
from skills.cvm.historical.metrics import rps as rps_metric
from skills.cvm.historical.engines import revenue as revenue_engine
...
def _mock_engines(monkeypatch):
    ...
    monkeypatch.setattr(
        "skills.cvm.historical.metrics.rps.price_series",
        lambda ticker, df, dt: list(MOCK_PRICES),
    )
    monkeypatch.setattr(
        "skills.cvm.historical.metrics.rps.revenue_periods",
        lambda company: list(MOCK_REVENUE_PERIODS),
    )
    monkeypatch.setattr(
        "skills.cvm.historical.metrics.rps.shares_periods",
...
class TestRpsAt:
    def test_basic_computation(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.rps.revenue_at", lambda c, d: 280e9
        )
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.rps.shares_at", lambda c, d: 13e9
        )
    ...
    def test_missing_revenue(self, monkeypatch):
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.rps.revenue_at", lambda c, d: None
        )
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.rps.shares_at", lambda c, d: 13e9
        )
    ...
    def test_missing_shares(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.rps.shares_at", lambda c, d: None
        )
        assert rps_metric.rps_at("PETR4", "2024-06-15") is None
    ...
    def test_zero_revenue_returns_none(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.rps.revenue_at", lambda c, d: 0
        )
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.rps.shares_at", lambda c, d: 13e9
        )
...
class TestPsrAt:
    def test_basic_computation(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.rps.price_at", lambda t, d: 38.0
        )
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.rps.revenue_at", lambda c, d: 280e9
        )
    ...
    def test_missing_price(self, monkeypatch):
        monkeypatch.setattr("skills.cvm.historical.metrics.rps.price_at", lambda t, d: None)
        monkeypatch.setattr("skills.cvm.historical.metrics.rps.revenue_at", lambda c, d: 280e9)
        monkeypatch.setattr("skills.cvm.historical.metrics.rps.shares_at", lambda c, d: 13e9)
        assert rps_metric.psr_at("PETR4", "2024-06-15") is None

    def test_zero_price_returns_none(self, monkeypatch):
        monkeypatch.setattr("skills.cvm.historical.metrics.rps.price_at", lambda t, d: 0.0)
        monkeypatch.setattr("skills.cvm.historical.metrics.rps.revenue_at", lambda c, d: 280e9)
        monkeypatch.setattr("skills.cvm.historical.metrics.rps.shares_at", lambda c, d: 13e9)
        assert rps_metric.psr_at("PETR4", "2024-06-15") is None
    ...
    def test_zero_revenue_returns_none(self, monkeypatch):
        ...
        monkeypatch.setattr("skills.cvm.historical.metrics.rps.price_at", lambda t, d: 38.0)
        monkeypatch.setattr("skills.cvm.historical.metrics.rps.revenue_at", lambda c, d: 0)
        monkeypatch.setattr("skills.cvm.historical.metrics.rps.shares_at", lambda c, d: 13e9)
        assert rps_metric.psr_at("PETR4", "2024-06-15") is None
