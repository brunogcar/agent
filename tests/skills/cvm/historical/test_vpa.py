...
class TestVpaAt:
    def test_basic_computation(self, monkeypatch):
        """vpa_at = price / (PL / shares)."""
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.vpa.price_at",
            lambda t, d: 38.0,
        )
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.vpa.pl_at",
            lambda c, d: 310e9,
        )
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.vpa.shares_at",
            lambda c, d: 13e9,
        )
        # VPA = 38.0 / (310e9 / 13e9) = 38.0 / 23.846 = 1.5935...
...
class TestVpaHistory:
    ...
    def test_step_function_pl_lookup(self, monkeypatch):
        ...
        assert result[0]["vpa"] == pytest.approx(35.0 / (290e9 / 13e9), rel=1e-3)
        # 2024-04-15 → most recent PL is 2024-03-31 (300e9)
        assert result[3]["pl"] == 300e9
        # 2024-06-15 → most recent PL is 2024-06-30 (310e9) — wait, 2024-06-15 < 2024-06-30
        # So most recent <= 2024-06-15 is 2024-03-31 (300e9)
    ...
    def test_no_pl_before_date_yields_none_vpa(self, monkeypatch):
        """If no PL snapshot <= date, vpa should be None."""
        monkeypatch.setattr(
            "skills.cvm.historical.metrics.vpa.price_series",
...
class TestPlEngine:
    def test_pl_at_finds_most_recent_snapshot(self, monkeypatch):
        ...
        # 2024-04-15 → most recent <= date is 2024-03-31 (300e9)
        assert pl_engine.pl_at("PETR4", "2024-04-15") == 300e9
        # 2024-07-01 → most recent <= date is 2024-06-30 (310e9)
        assert pl_engine.pl_at("PETR4", "2024-07-01") == 310e9
        # 2023-12-31 → exactly the DFP snapshot (290e9)
    ...
    def test_pl_periods_merges_dfp_and_itr(self, monkeypatch):
        ...
        assert len(result) == 5  # 2 DFP + 3 ITR
        # Sorted oldest-first
        assert result[0] == {"date": "2023-12-31", "pl": 290e9}
        assert result[1] == {"date": "2024-03-31", "pl": 300e9}
        assert result[2] == {"date": "2024-06-30", "pl": 310e9}
        assert result[3] == {"date": "2024-09-30", "pl": 330e9}
        assert result[4] == {"date": "2024-12-31", "pl": 380e9}

    def test_pl_periods_dedupes_same_date(self, monkeypatch):
        """If DFP and ITR both have the same date (Dec 31), should dedupe."""
        fake_dfp = {"2024-12-31": {"value": 380e9, "year": 2024}}
        ...
        assert result[0] == {"date": "2024-12-31", "pl": 380e9}
...
class TestMetricRegistry:
    def test_vpa_registered(self):
        from skills.cvm.historical.metrics import METRICS
        assert "vpa" in METRICS
        assert "P/VPA" in METRICS["vpa"]

    def test_pe_still_registered(self):
        from skills.cvm.historical.metrics import METRICS
