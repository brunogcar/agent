"""Tests for skills/cvm/calculations/engines/total_tax.py.

Flow engine (DVA grupo='DVA', codigo 8.2 -- Impostos, Taxas e Contribuições
/ total tax burden, TTM derivation from DFP + ITR cumulative). Mocks the
internal _get_dfp_total_tax + _get_itr_total_tax functions via
...
# -- total_tax_at() tests (TTM derivation) ------------------------------------

class TestTotalTaxAt:
    def test_basic_computation(self, monkeypatch):
        """total_tax_at should derive TTM via DFP - ITR_prior + ITR_current.

        TTM at 2024-04-15 = DFP_2023 - ITR_2023_Q1 + ITR_2024_Q1
                          = -90e9 - (-22e9) + (-25e9)
                          = -93e9
        """
        monkeypatch.setattr(tt_engine, "_get_dfp_total_tax", lambda c: FAKE_DFP)
        monkeypatch.setattr(tt_engine, "_get_itr_total_tax", lambda c: FAKE_ITR)

        result = tt_engine.total_tax_at("PETR4", "2024-04-15")
    ...
    def test_missing_company(self, monkeypatch):
        ...
        """
        monkeypatch.setattr(tt_engine, "_get_dfp_total_tax", lambda c: {})
        monkeypatch.setattr(tt_engine, "_get_itr_total_tax", lambda c: {})

        assert tt_engine.total_tax_at("UNKNOWN", "2024-06-30") is None
    ...
    def test_returns_dfp_when_no_itr_before_date(self, monkeypatch):
        ...
        assert tt_engine.total_tax_at("PETR4", "2021-01-15") == -60e9
    ...
    def test_no_prior_year_dfp_returns_none(self, monkeypatch):
        ...
        assert tt_engine.total_tax_at("PETR4", "2024-04-15") is None
    ...
    def test_ttm_at_exact_period_end(self, monkeypatch):
        ...
        result = tt_engine.total_tax_at("PETR4", "2024-03-31")
        assert result == pytest.approx(-93e9, rel=1e-6)


# -- total_tax_periods() tests ------------------------------------------------

class TestTotalTaxPeriods:
    def test_periods(self, monkeypatch):
        """total_tax_periods returns list of {date, ttm_total_tax}."""
        fake_dfp = {
        ...
        result = tt_engine.total_tax_periods("PETR4")
        ...
        for entry in result:
            ...
            assert "ttm_total_tax" in entry
            assert isinstance(entry["ttm_total_tax"], float)
    ...
    def test_periods_empty_when_no_data(self, monkeypatch):
        ...
        assert tt_engine.total_tax_periods("UNKNOWN") == []


# -- Registry tests ----------------------------------------------------------

class TestTotalTaxRegistry:
    def test_registry(self):
        """Engine should be registered with correct name, category, quantity."""
        from skills.cvm.calculations._registry import ENGINES
        assert "total_tax" in ENGINES
        spec = ENGINES["total_tax"]
        assert spec.name == "total_tax"
        assert spec.category == "dva"
        assert spec.quantity == "ttm_total_tax"
        assert spec.at_fn is tt_engine.total_tax_at
