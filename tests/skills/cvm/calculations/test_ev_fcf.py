...
class TestEvFcfAt:
    def test_basic_computation(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "skills.cvm.calculations.metrics.ev_fcf.operating_cf_periods",
            lambda c: [{"date": "2024-06-30", "ttm_fco": 280e9}],
        )
        monkeypatch.setattr(
            "skills.cvm.calculations.metrics.ev_fcf.investing_cf_periods",
    ...
    def test_misaligned_periods_returns_none(self, monkeypatch):
        ...
        monkeypatch.setattr("skills.cvm.calculations.metrics.ev_fcf.debt_at", lambda c, d: 200e9)
        monkeypatch.setattr("skills.cvm.calculations.metrics.ev_fcf.cash_at", lambda c, d: 80e9)
        assert ev_fcf_metric.ev_fcf_at("PETR4", "2024-07-15") is None
