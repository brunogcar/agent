...
def register_all_tools(mcp: FastMCP) -> int:
    """
    Discover and register all @tool-decorated functions in tools/, data_sources/, and skills/.
    ...
    data_sources/ -- scanned at the TOP LEVEL ONLY (flat modules).
    Only data_sources/dispatcher.py is expected to have @tool here.
    Sub-packages (data_sources/cvm/, etc.) are pure Python modules --
    they are NOT scanned directly. The dispatcher imports them internally
    via its own domain discovery mechanism.

    skills/ -- scanned at the TOP LEVEL ONLY (flat modules).
    Only skills/dispatcher.py is expected to have @tool here (future).
    Currently empty — the skills/ layer will contain domain expertise
    packages that combine data sources with reasoning.
    """
    ...
    # ── Scan data_sources/ (top-level flat modules only) ─────────────────────
    # Only imports modules directly in data_sources/ (not sub-packages).
    # dispatcher.py lives here and is the sole @tool entry point for all data sources.
    try:
        import data_sources
        for finder, module_name, is_pkg in pkgutil.iter_modules(data_sources.__path__):
            if is_pkg:
                continue
            full_name = f"data_sources.{module_name}"
            try:
        ...
        print(f"[registry] Scanned data_sources/ for @tool functions", file=sys.stderr)
    except ImportError:
        print("[registry] data_sources/ package not found, skipping", file=sys.stderr)

    # ── Scan skills/ (top-level flat modules only) ────────────────────────────
    # Only imports modules directly in skills/ (not sub-packages).
    # dispatcher.py lives here and is the sole @tool entry point for all skills.
    # [v1.5] skills/ is currently empty — the expertise layer will be built
    # on top of data_sources/ once they're stable.
    try:
        import skills
        for finder, module_name, is_pkg in pkgutil.iter_modules(skills.__path__):
            if is_pkg:
                # Sub-packages -- skip, dispatcher loads them
                continue
    ...
    print(f"[registry] Registered {registered} tools total (tools/ + data_sources/ + skills/)", file=sys.stderr)
    return registered
