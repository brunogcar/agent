...
print(f"   router: model={router_role['model']}, timeout={router_role['timeout']}s ✓")
available = llm.is_available()
print(f"   LM Studio: {'reachable ✓' if available else 'not running (skipping live tests)'}")
...
print(f"   ChromaDB: {total} memories across {len(stats)} collections ✓")
...
assert r["status"] == "success"
print(f"   visualize(chart) → {r['html_path'].split(chr(92))[-1]} ✓")
...
if available:
    ...
    r = agent(role="classify",
              ...
              content="What is 2 + 2?")
    assert r["status"] == "success"
    print(f"   agent(classify)  → '{r['text']}' [{r['elapsed']}s] ✓")
    ...
    assert r["status"] == "success"
    print(f"   agent(summarize) → '{r['text'][:80]}' ✓")
else:
