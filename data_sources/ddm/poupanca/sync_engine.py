...
the per-source config (POUPANCA_CATALOG, fetcher fn, parser pipeline,
INSERT SQL, row mapper) + the sync_index() entry point (which has
poupanca-specific slug validation + the matrix-flatten pipeline).
"""
...
from data_sources.ddm.poupanca.fetcher import (
    fetch_poupanca_page, flatten_matrix_to_observations, parse_matrix_only,
)


class _SyncEngine(BaseDDMSyncEngine):
    """Poupanca-specific sync engine config (SOURCE_NAME for log prefix)."""
...
def _parse_pipeline(html: str) -> list[dict]:
    """poupanca pipeline: parse_matrix_only -> flatten_matrix_to_observations."""
...
def sync_index(slug: str, force: bool = False) -> dict:
    ...
    _SyncEngine._progress(
        f"[ddm.poupanca] Index {slug}: {len(rows)} observations derived + synced"
    )
...
def sync_all(force: bool = False) -> dict:
    ...
    return _SyncEngine.sync_multi_page(
        catalog=POUPANCA_CATALOG,
        fetch_fn=fetch_poupanca_page,
