"""data_sources/ddm/poupanca/query_engine.py -- Read-only queries against poupanca.db.

Functions:
  - poupanca_history(slug, limit=60)  - last N derived observations (ascending)
  - last_value(slug)                   - latest derived observation for an index
  - monthly_matrix(slug)               - matrix dict (year x month, NO Ano column)
  - search(query, limit=10)            - LIKE search over POUPANCA_CATALOG
...
from data_sources.ddm.poupanca.fetcher import (
    fetch_poupanca_page, parse_matrix_only,
)


def poupanca_history(slug: str = "", limit: int = 60) -> dict:
    ...
    try:
        rows = list(reversed(conn.execute(
            "SELECT ref_date, month_value, acumulado_no_ano, acumulado_12m "
        ...
        if not rows:
            meta = POUPANCA_CATALOG.get(slug)
        ...
        return {
            ...
            "observations": [
                {"ref_date":          r["ref_date"],
                 ...
                 "acumulado_no_ano":  r["acumulado_no_ano"],
...
def last_value(slug: str = "") -> dict:
    ...
    Returns:
        {"status": "ok", "slug": <str>, "name": ..., "unit": ...,
         "ref_date": ..., "month_value": ..., "acumulado_no_ano": ...,
         "acumulado_12m": ...}
    """
    if not slug:
        return {"status": "error", "error": "slug is required"}
    ...
    try:
        ...
        return {
            ...
            "acumulado_no_ano":  row["acumulado_no_ano"],
...
def monthly_matrix(slug: str = "") -> dict:
    ...
    page = fetch_poupanca_page(slug, force=False)
...
def search(query: str = "", limit: int = 10) -> dict:
    """Search POUPANCA_CATALOG by name/slug fragment (case-insensitive).
...
def summary() -> dict:
    ...
    try:
        rows = conn.execute(
            ...
            "FROM poupanca_catalog ORDER BY category, slug"
        ).fetchall()
        if rows:
            return {
                "status":  "ok",
