"""data_sources/ddm/poupanca/fetcher.py -- HTTP fetcher + HTML parser for DDM Poupanca.

Handles:
  - HTTP GET to www.dadosdemercado.com.br/indices/poupanca (no auth, no JS)
  ...
  - Boundary normalizations:
      ...
      * "0,67"             -> 0.67        (Brazilian comma decimal)
      * "--"               -> None        (missing values)

The poupanca page does NOT ship a historical monthly table. The matrix is
the ONLY data source. The historical series (month_value, acumulado_no_ano,
acumulado_12m) is DERIVED from the matrix at parse time:
  ...
  - acumulado_no_ano   = SUM of all months in same year UP TO that month
                         ...
                         Matches: SUM(FILTER(B:B, YEAR(A:A)=YEAR(d), A:A<=d))
  - acumulado_12m      = SUM of the last 12 months INCLUDING the current month
                         (rolling 12-month cumulative return).
                         Matches: SUM(FILTER(B:B, A:A<=d, A:A>=d-365))
...
return. This is the KEY difference from juros (which uses AVERAGE because
juros rates are quoted as annualized daily rates - averaging produces the
period-average rate).

NO local database writes - this module only fetches + parses.
sync_engine.py stores derived observations to poupanca.db.
"""
...
# Thread-safety primitives (mirror bcb/sgs + ddm/juros fetcher):
...
def _parse_data_value(td: str) -> float | None:
    ...
    DDM matrix cells look like: <td class="right" data-value="0.67">0,67%</td>
...
def parse_matrix_only(html: str) -> dict:
    """Parse the monthly matrix table (the ONLY table on a DDM poupanca page).

    The poupanca page ships just 1 table with id="index-values":
    ...
    # (e.g. "Ano" if it accidentally slipped in - poupanca pages shouldn't
...
def flatten_matrix_to_observations(matrix: dict) -> list[dict]:
    """DERIVE the historical series from the monthly matrix using SUM.

    The poupanca page does NOT ship a historical table - only the year x
    month matrix. This function flattens the matrix into a sorted list of
    monthly observations and computes the two derived SUMs:

      - month_value        = cell value (monthly yield % for that month)
      - acumulado_no_ano   = SUM of all months in the same year UP TO that
                             month (year-to-date cumulative return).
                             Matches: SUM(FILTER(B:B, YEAR(A:A)=YEAR(d), A:A<=d))
      - acumulado_12m      = SUM of the last 12 months INCLUDING the current
                             month (rolling 12-month cumulative return).
                             Matches: SUM(FILTER(B:B, A:A<=d, A:A>=d-365))
    ...
    Google Sheet formula which sums whatever rows match the filter (could
    be fewer than 12 at the start of the series).

    IMPORTANT: Poupanca uses SUM (not AVERAGE like juros). This is because
    the monthly yield is a percentage return - summing produces the
    cumulative return over the period. Juros uses AVERAGE because the
    monthly cell is a daily rate quoted as an annualized % - averaging
    produces the period-average rate.

    Returns a list of dicts sorted ASCENDING by ref_date:
      [{"ref_date": "YYYY-MM", "month_value": <float|None>,
        "acumulado_no_ano": <float|None>, "acumulado_12m": <float|None>}, ...]
    """
    if not matrix:
        return []
    ...
    # Iterate years ascending so the rolling SUM sees the right order.
    ...
    for i, (ref_date, val) in enumerate(flat):
        ...
        # acumulado_no_ano: SUM of all months in same year UP TO this one.
        ...
        acumulado_no_ano = sum(same_year_vals) if same_year_vals else None

        # acumulado_12m: SUM of the last 12 months INCLUDING current.
        # Slice the flat list to [max(0, i-11), i+1] and sum all non-None.
        ...
        acumulado_12m = sum(window_vals) if window_vals else None

        observations.append({
            "ref_date":          ref_date,
            "month_value":       val,
            "acumulado_no_ano":  acumulado_no_ano,
            "acumulado_12m":     acumulado_12m,
        })

    return observations


def fetch_poupanca_page(slug: str, force: bool = False) -> dict:
    """Fetch the HTML page for an index.

    Args:
        slug:  DDM poupanca slug (e.g. 'poupanca').
        force: Bypass cache.
    ...
    _progress(f"[ddm.poupanca] Fetching poupanca page {slug} ({url})")
