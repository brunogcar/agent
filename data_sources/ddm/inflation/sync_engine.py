...
def sync_index(slug: str, force: bool = False) -> dict:
    ...
    _progress(f"[ddm.inflation] Index {slug}: {len(rows)} observations synced")
...
def sync_all(force: bool = False) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        future_to_slug = {
            executor.submit(fetch_index_page, slug, force): slug
            for slug in slugs
        }
        for future in as_completed(future_to_slug):
            ...
            if page.get("status") != "ok":
                ...
                index_failed += 1
                continue
            fetch_results[slug] = parse_historical_table(page.get("html", ""))
    ...
    _progress(f"[ddm.inflation] sync_all: {index_synced}/{len(slugs)} indices, "
