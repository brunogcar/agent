"""data_sources/cvm/cgvn/sync_engine.py -- Download + parse CGVN ZIPs from CVM.

Downloads annual ZIP files, extracts CSVs, parses into SQLite.
Pattern follows VLMO/DFP/ITR/FRE sync engines.

Supports:
  sync(year=2025)           -- sync single year
  sync(full_history=True)   -- sync all years from CGVN_FIRST_YEAR to current
"""
...
# CVM started CGVN filings in 2018
CGVN_FIRST_YEAR = 2018
...
def sync(
    ...
    Args:
        ...
        full_history: Sync all years from CGVN_FIRST_YEAR to current.
...
def _sync_full_history(force: bool = False) -> dict:
    """Sync all years from CGVN_FIRST_YEAR to current."""
    current_year = datetime.now().year
    years_synced = []
    years_skipped = []
    total_rows = 0
    ...
    for yr in range(CGVN_FIRST_YEAR, current_year + 1):
        result = _sync_single_year(yr, force=force)
        if result.get("status") == "ok":
            years_synced.append(yr)
            total_rows += result.get("practice_rows", 0)
    ...
    return {
        ...
        "total_practice_rows": total_rows,
        "errors": errors,
    }


def _sync_single_year(year: int, force: bool = False) -> dict:
    """Download + parse a single year of CGVN data."""
    ...
    # Download ZIP
    url = f"{CGVN_BASE_URL}/cgvn_cia_aberta_{year}.zip"
    print(f"[cgvn] Downloading {url}...")
    try:
    ...
    #   cgvn_cia_aberta_YYYY.csv           (documents)
    #   cgvn_cia_aberta_praticas_YYYY.csv  (practices)
    doc_file = None
    prac_file = None
    for name in zf.namelist():
        lower = name.lower()
        if lower.endswith(".csv"):
            if "cgvn_cia_aberta_praticas" in lower:
                prac_file = name
            elif "cgvn_cia_aberta" in lower:
                doc_file = name
    ...
    try:
        ...
        conn.execute(
            "DELETE FROM cgvn_practices WHERE substr(Data_Referencia, 1, 4) = ?",
            (year_prefix,),
        )
        conn.execute(
            "DELETE FROM cgvn_documents WHERE substr(Data_Referencia, 1, 4) = ?",
            (year_prefix,),
        )
        ...
        if doc_file:
            doc_rows = _parse_and_insert_csv(
                conn, zf, doc_file, "cgvn_documents",
                ["Categoria", "CNPJ_Companhia", "Codigo_CVM", "Data_Entrega",
                 "Data_Fim_Exercicio_Social", "Data_Inicio_Exercicio_Social",
                 "Data_Referencia", "ID_Documento", "Link_Download",
                 "Nome_Empresarial", "Versao", "Motivo_Reapresentacao"]
            )

        # Parse practices CSV (the valuable one)
        prac_rows = _parse_and_insert_csv(
            conn, zf, prac_file, "cgvn_practices",
            ["CNPJ_Companhia", "Data_Referencia", "ID_Documento",
             "Nome_Empresarial", "Versao", "ID_Item",
             "Pratica_Recomendada", "Pratica_Adotada", "Capitulo",
        ...
        conn.execute(
            ...
            (datetime.now().isoformat(), year, prac_rows),
        )
        conn.commit()
        ...
        return {
            ...
            "practice_rows": prac_rows,
            "total_rows": doc_rows + prac_rows,
...
def _parse_and_insert_csv(
    ...
    print(f"[cgvn] {table}: {rows_inserted} rows inserted")
