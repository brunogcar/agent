...
def sync(
    ...
    Args:
        ...
        full_history: Sync all years from FIRST_YEAR (2011) to current.
    ...
    def _log(msg: str) -> None:
        if verbose:
            print(f"[itr] {msg}", file=sys.stderr, flush=True)

    _log(f"Starting ITR sync — {len(years_to_sync)} year(s): {years_to_sync[0]}..{years_to_sync[-1]}")
...
def _process_row(
    ...
    # (scoped to period, not whole empresa — safer for partial failures).
