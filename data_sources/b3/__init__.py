"""data_sources/b3/__init__.py -- B3 domain root manifest and sub-domain router.

Auto-discovered by data_sources/dispatcher.py. Scans data_sources/b3/ for
sub-domain packages (api, dividends, etc.) with MANIFEST + route().

Sub-domains:
  api -- B3 market data: instruments (tickers, ISIN, company names), trades, derivatives
"""
...
MANIFEST = {
    ...
    "description":     (
        ...
        "api: market data (instruments, trades, derivatives) via paginated JSON API."
    ),
...
def _discover_sub_domains() -> dict[str, Any]:
    """Scan data_sources/b3/ for sub-domain packages with MANIFEST + route()."""
    global _SUB_DOMAINS
    ...
    for item in sorted(here.iterdir()):
        ...
        if not init_file.exists():
            continue
        module_path = f"data_sources.b3.{item.name}"
        try:
...
def route(sub_domain: str = "", mode: str = "", **kwargs: Any) -> Any:
    """Route data_source(domain="b3", sub_domain=..., mode=...) calls."""
    sub_domains = _discover_sub_domains()

    if not sub_domains:
        return {"status": "error", "error": "No b3 sub-domains found in data_sources/b3/"}
